...
public import Foundations.QuasiBorel.SFinite.Standard.Joint
    ...
    let joint : Standard.JointFamily (Space.product left right)
      ...
      random := {
        ...
        mapRandom := by
          ...
          exact ⟨left.reparam
            (MeasurableMap.comp
              (Foundations.Measure.Space.first_measurable borel family.source) measurable) accepted.1,
            family.random.mapRandom (MeasurableMap.comp
              (Foundations.Measure.Space.second_measurable borel family.source) measurable)⟩
      }
