...
@[expose] noncomputable def bind (law : Law domain)
  ...
  let family := Classical.choice ((zeroExtend kernel).mapRandom law.presentation.accepted)
...
@[expose] noncomputable def Family.bind {laws : Carrier → Law domain}
  ...
  let continuation := Classical.choice ((zeroExtend kernel).mapRandom family.accepted)
  refine {
