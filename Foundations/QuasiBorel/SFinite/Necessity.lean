...
theorem total_generator_requires_inhabitant {α : Type u} (random : Carrier → α) :
    Nonempty α :=
  ⟨random Foundations.Real.Construction.Dedekind.zero⟩
...
theorem no_total_generator_empty : ¬Nonempty (Carrier → PEmpty.{u + 1}) := by
  rintro ⟨random⟩
  exact (random Foundations.Real.Construction.Dedekind.zero).elim
...
theorem zero_measure_without_total_generator :
    Nonempty (Foundations.Measure.Measure
      (Foundations.Measure.Space.discrete PEmpty.{u + 1})) ∧
      ¬Nonempty (Carrier → PEmpty.{u + 1}) :=
  ⟨⟨Foundations.Measure.Measure.zero _⟩, no_total_generator_empty⟩
...
theorem distinct_presentations_same_measure (space : Space realSource) :
  ...
  let point := Foundations.Real.Construction.Dedekind.zero
