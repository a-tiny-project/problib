...
public import Foundations.QuasiBorel.SFinite.Generator
...
namespace Foundations.QuasiBorel.SFinite.Generator
...
@[expose] def seedSpace (generator : Generator space) : Foundations.Measure.Space generator.Seed :=
  Foundations.Measure.Space.comap (fun seed : generator.Seed => seed.val) borel
...
@[expose] noncomputable def decoder (generator : Generator space) :
  ...
  mapRandom := by
    ...
    let fallback := generator.decode (random Foundations.Real.Construction.Dedekind.zero)
    ...
    have accepted := recover.mapRandom ((withFailure space).reparam inclusion generator.accepted)
...
theorem decode_measurable (generator : Generator space) :
  ...
  rw [Space.toMeasurable_ofEmbedding generator.seedEmbedding] at measurable
  exact @measurable
...
end Foundations.QuasiBorel.SFinite.Generator
...
theorem measurable (generator : Generator.{u, v} space) :
  ...
  generator.random.toMeasurable_ofEmbedding generator.standard.embeddingReal
...
@[expose] noncomputable def toPartial (generator : Generator.{u, v} space) :
  ...
  exact if inhabited : Nonempty generator.Seed then
    ...
    { random := fun seed => Sum.inr
      ...
      accepted := SumRandom.inr (generator.random.mapRandom
