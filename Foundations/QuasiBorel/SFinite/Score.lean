...
public import Foundations.QuasiBorel.SFinite.Mass
public import Foundations.Measure.Kernel.Density
...
private def scoreSeed : Carrier := Foundations.Real.Construction.Dedekind.zero
...
private theorem scoreDensity_measurable :
    ENNRealMeasurable (Foundations.Measure.Space.product ennrealBorel borel)
      (fun pair => pair.1) :=
  ENNRealMeasurable.identity.comp (Foundations.Measure.Space.first_measurable ennrealBorel borel)

private noncomputable def scoreSource : Kernel ennrealBorel borel :=
  scoreBase.withDensity scoreBase_sfinite (fun weight _ => weight) scoreDensity_measurable

private noncomputable def scoreSource_sfinite : Kernel.IsSFinite scoreSource :=
  scoreBase_sfinite.withDensity (fun weight _ => weight) scoreDensity_measurable
