...
public import Foundations.QuasiBorel.Sum
...
variable {Ω : Type u} {source : Foundations.Measure.Space Ω}
...
theorem SumRandom.measurable {random : Ω → _root_.Sum left.Carrier right.Carrier}
    ...
    Foundations.Measure.MeasurableMap source
      (Foundations.Measure.Space.sum left.toMeasurable right.toMeasurable) random := by
  induction accepted with
  | inl accepted =>
      exact Foundations.Measure.MeasurableMap.comp
        (Foundations.Measure.MeasurableMap.inl _ _) (Space.random_measurable accepted)
  | inr accepted =>
      exact Foundations.Measure.MeasurableMap.comp
        (Foundations.Measure.MeasurableMap.inr _ _) (Space.random_measurable accepted)
  | reparam measurable _ induction =>
      exact Foundations.Measure.MeasurableMap.comp induction measurable
  | piecewise measurable _ induction =>
      exact Foundations.Measure.MeasurableMap.countablePiecewise measurable induction
...
theorem Space.toMeasurable_sum
    ...
    (sum left right).toMeasurable =
      Foundations.Measure.Space.sum left.toMeasurable right.toMeasurable := by
