...
namespace Foundations.QuasiBorel.RandomFamily
...
private noncomputable def coding :
    MeasurableEmbedding
      (Foundations.Measure.Space.product (Foundations.Measure.Space.discrete Nat) borel) borel :=
  (StandardBorel.natural.product StandardBorel.real).embeddingReal

private noncomputable def decode : Carrier → Nat × Carrier :=
  coding.retract (0, Foundations.Real.Construction.Dedekind.zero)

private theorem decode_measurable :
    MeasurableMap borel
      (Foundations.Measure.Space.product (Foundations.Measure.Space.discrete Nat) borel) decode :=
...
theorem select_measurable (index : Nat) : MeasurableMap borel borel (select index) :=
  MeasurableMap.comp coding.measurable
    (Foundations.Measure.Space.pair_measurable
      (MeasurableMap.constant borel (Foundations.Measure.Space.discrete Nat) index)
...
theorem join_random {space : Space (Source.ofMeasurable borel)}
  ...
  have firstMeasurable : MeasurableMap borel (Foundations.Measure.Space.discrete Nat)
      (fun seed => (decode seed).1) := MeasurableMap.comp
    (Foundations.Measure.Space.first_measurable (Foundations.Measure.Space.discrete Nat) borel)
    decode_measurable
  have secondMeasurable : MeasurableMap borel borel (fun seed => (decode seed).2) :=
    MeasurableMap.comp
    (Foundations.Measure.Space.second_measurable (Foundations.Measure.Space.discrete Nat) borel)
    decode_measurable
  have partition : (Source.ofMeasurable borel).Partition (fun seed => (decode seed).1) := by
    intro index
    exact firstMeasurable (show (Foundations.Measure.Space.discrete Nat).Measurable
...
end Foundations.QuasiBorel.RandomFamily
