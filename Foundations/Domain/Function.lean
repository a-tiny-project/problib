...
public import Foundations.Domain.FixedPoint

namespace Foundations.Domain.CPO
 ...
 theorem continuous_sup {D : CPO α} {E : CPO β} (f : Nat → α → β)
  ...
  preserves := by
    ...
    · apply E.supLeast
      ...
      apply E.supLeast
      intro m
      exact E.trans (E.belowSup (fun n => f n (c m)) _ n)
        (E.belowSup (fun m => E.sup (fun n => f n (c m)) (hc (c m))) _ m)
    · apply E.supLeast
      intro m
      apply E.supLeast
      intro n
      exact E.trans ((hf n).monotone (D.belowSup c hd m))
        (E.belowSup (fun n => f n (D.sup c hd)) _ n)
...
@[expose] def subtype (D : CPO α) (p : α → Prop) (hp : D.Admissible p) :
  ...
  belowSup := fun c _ n => D.belowSup (fun n => (c n).val) _ n
  supLeast := fun _ _ b h => D.supLeast _ _ b.val h
  ...
  preserves := by
    ...
    · apply E.supLeast
      ...
      exact E.trans
        ...
        (E.belowSup (fun k => (c k).1.val (c k).2) _ (max n m))
    · apply E.supLeast
      intro n
      exact E.trans ((c n).1.property.monotone
        (D.belowSup (fun n => (c n).2) _ n))
        (E.belowSup (fun n => (c n).1.val
