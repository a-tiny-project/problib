...
private theorem positivePart_sub_negativePart (value : Rat) :
...
theorem Equivalent.expectation_eq {α : Type u} {left right : FiniteMeasure α}
  ...
  calc
    left.expectation observable =
      ...
      apply expectation_congr
      intro value
      exact (positivePart_sub_negativePart (observable value)).symm
    ...
    _ = right.expectation observable := by
      apply expectation_congr
      intro value
      exact positivePart_sub_negativePart (observable value)
