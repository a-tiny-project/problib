...
namespace Foundations.Probability.FiniteExample
...
theorem scoredCoin_total : (fairCoin.score headsLikelihood).total = 1 := by
...
theorem scoredCoin_false_mass : (fairCoin.score headsLikelihood).mass false = 0 := by
  rw [FinitePMF.score_mass, fairCoin_mass_false]
  simp [headsLikelihood]

theorem scoredCoin_true_mass : (fairCoin.score headsLikelihood).mass true = 1 := by
...
end Foundations.Probability.FiniteExample
