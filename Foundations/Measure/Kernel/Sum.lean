...
theorem IsFinite.copair {first : Kernel left target} {second : Kernel right target}
  ...
  refine ⟨⟨ENNReal.add firstBound secondBound,
    ENNReal.addFinite firstBoundFinite secondBoundFinite, ?_⟩⟩
  intro input
  cases input with
  | inl value =>
    apply ENNReal.leTrans (firstBounded value)
    simpa only [ENNReal.addZero] using
      ENNReal.addLeAddLeft (ENNReal.zeroLe secondBound) firstBound
  | inr value =>
    apply ENNReal.leTrans (secondBounded value)
    simpa only [ENNReal.zeroAdd] using
      ENNReal.addLeAddRight (ENNReal.zeroLe firstBound) secondBound
