...
import Foundations.Measure.Dynkin.PiLambda
...
public theorem verticalMap_measurable (input : alpha) :
...
public theorem verticalSection_measurable {set : Set (alpha × beta)}
  ...
  exact Space.pair_measurable (verticalMap_measurable input)
...
private theorem verticalSection_complement_apply
  ...
  exact (ENNReal.addSubCancelLeft
