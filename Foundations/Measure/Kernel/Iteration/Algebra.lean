...
public theorem leRefl (kernel : Kernel source target) : le kernel kernel :=
  fun _ _ _ => ENNReal.leRefl _

public theorem leTrans {first second third : Kernel source target}
    (firstSecond : le first second) (secondThird : le second third) :
    le first third :=
  fun input set measurable =>
    ENNReal.leTrans (firstSecond input set measurable) (secondThird input set measurable)

public theorem leAntisymm {left right : Kernel source target}
    (leftRight : le left right) (rightLeft : le right left) : left = right := by
  apply Kernel.ext_measurable
  intro input set measurable
  exact ENNReal.leAntisymm (leftRight input set measurable) (rightLeft input set measurable)
...
public theorem add_le_add {left₁ right₁ left₂ right₂ : Kernel source target}
  ...
  rw [Kernel.add_apply, Kernel.add_apply,
    ...
    Measure.add_apply_measurable _ _ measurable]
  exact ENNReal.addLeAdd (first input set measurable) (second input set measurable)
...
public theorem sum_split_head (kernels : Nat → Kernel source target) :
  ...
  rw [Kernel.sum_apply, Kernel.add_apply, Kernel.sum_apply,
    ...
    Measure.sum_apply _ setMeasurable]
  have tailEq := ENNReal.partialSumAddTail
    (fun index => kernels index input set) 1
  rw [ENNReal.partialSum, ENNReal.partialSum, ENNReal.zeroAdd] at tailEq
  rw [← tailEq]
