...
public import Foundations.Measure.Kernel.Iteration.Algebra
...
public theorem prefixApproximant_monotone (step : Kernel source source)
  ...
  exact ENNReal.partialSumMonotone _ included
...
public theorem prefixApproximant_le_loop (step : Kernel source source)
  ...
  exact ENNReal.leISup (fun index => prefixApproximant step exit index input set) count
...
public theorem loop_least (step : Kernel source source)
  ...
  have stepBounded : ∀ count, le (stepApproximant step exit count) candidate := by
    ...
    | succ count induction =>
        ...
        exact leTrans (add_le_add (leRefl exit) (comp_le_comp_right step induction)) prefixed
