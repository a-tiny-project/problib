...
public import Foundations.Measure.Kernel.Iteration
public import Foundations.Measure.Normalization
...
public theorem loop_retry_reconstruct (weight : alpha → ENNReal)
  ...
  have weightFinite := ((retry_isFinite weight measurable exit conservative).1.measure input).univFinite
  ...
  rw [Measure.add_apply_measurable _ _ setMeasurable,
    ...
    Measure.smul_apply_measurable _ _ setMeasurable]
  rw [ENNReal.mulComm (exit input Set.univ), ENNReal.mulComm (weight input),
    ← ENNReal.mulAdd, ENNReal.addComm (exit input Set.univ), conservative input,
    ENNReal.mulOne]
...
public theorem retry_exit_normalizable (weight : alpha → ENNReal)
  ...
  ⟨(retry_isFinite weight measurable exit conservative).2.measure input,
    ENNReal.zeroLtIffNeZero.mp positive⟩
...
public theorem retry_vanishing_continuation (weight : alpha → ENNReal)
    (measurable : ENNRealMeasurable source weight) (exit : Kernel source target)
    (conservative : ∀ input,
      ENNReal.add (weight input) (exit input Set.univ) = ENNReal.one)
