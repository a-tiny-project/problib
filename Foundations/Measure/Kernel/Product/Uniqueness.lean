...
public import Foundations.Measure.Product.Generator
...
private theorem finite_mul {left right : ENNReal}
    (leftFinite : ENNReal.Finite left)
    (rightFinite : ENNReal.Finite right) :
    ENNReal.Finite (ENNReal.mul left right) := by
  rcases ENNReal.existsFiniteOfFinite leftFinite with ⟨leftValue, rfl⟩
  rcases ENNReal.existsFiniteOfFinite rightFinite with ⟨rightValue, rfl⟩
  exact True.intro
...
public noncomputable def prod {left : Measure source}
  ...
  finite := by
    ...
    exact finite_mul (leftFinite.finite index) (rightFinite.finite index)
