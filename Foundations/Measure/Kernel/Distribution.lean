...
public import Foundations.Measure.Kernel.Generator
...
@[expose] public noncomputable def ofDistributionFunction (family : α → DistributionFunction)
  ...
  ofGenerator (fun input => (family input).measure)
    (fun input => (family input).measure_isProbability.toFinite)
    (by
    ...
    unitInitials unitBorel_generatedInitials unitInitials_pi
    (by
      ...
      rw [same]
      exact ofRealMeasurable.comp (MeasurableMap.comp unitInclusionMeasurable (slices point)))
