...
public import Foundations.Measure.Distribution.Bounds.Measurable
public import Foundations.Measure.Distribution.Joint
...
namespace Foundations.Measure.Kernel.Randomizer

open Foundations.Measure.Real Foundations.Real Foundations.Real.Construction
...
theorem bounds_measurable (kernel : Kernel source unitBorel) (index : Nat) :
  ...
  MeasurableMap.comp unitClampMeasurable
    (kernel.measurable (unitInitialMeasurable (unitRationalBasis index))).measurableMap
...
theorem distribution_measure (kernel : Kernel source unitBorel)
  ...
  apply finiteMeasure_ext_unitInitial (distribution kernel input).measure_isProbability.toFinite
    (normalized input).toFinite
  intro point
  apply ENNReal.leAntisymm
  · apply le_measure_unitInitial_of_rightDense (normalized input).toFinite unitRationalBasis
      (fun _ _ less => by
    ...
    · intro index active
      ...
      have embedded := ENNReal.ofRealMonotone below
  ...
  · rw [DistributionFunction.measure_initial]
    have below := DistributionFunction.le_ofUpperBounds unitRationalBasis
      ...
      (fun index active => unitClamp_mono (kernel input |>.mono
        (fun _ included => Dedekind.leTrans included active.1)))
    have embedded := ENNReal.ofRealMonotone below
    rw [ofReal_unitClamp_of_le ((normalized input).apply_le_one _)] at embedded
    exact embedded
...
end Foundations.Measure.Kernel.Randomizer
