...
public import Foundations.Measure.Extended.Ratio
...
public noncomputable def ofCommonFiniteReference
  ...
  densityMeasurable := kernelDerivative.densityMeasurable.densityRatio referenceDerivative.densityMeasurable
  reconstruct := by
    intro input
    ...
    have leftMeasurable : ENNRealMeasurable target left :=
      jointlyMeasurable_slice kernelDerivative.densityMeasurable input
    have rightMeasurable : ENNRealMeasurable target right :=
      jointlyMeasurable_slice referenceDerivative.densityMeasurable input
    ...
    have factorization : (base input).withDensity left =
      ...
      rw [← (base input).withDensity_withDensity rightMeasurable derivative.densityMeasurable,
        ← referenceDerivative.reconstruct input, ← kernelDerivative.reconstruct input]
      exact derivative.reconstruct_eq
    have equal := Measure.ae_eq_of_withDensity_eq (Measure.SigmaFinite.ofFinite (finite input))
      leftMeasurable (rightMeasurable.mul derivative.densityMeasurable) factorization
