...
public import Foundations.Measure.Caratheodory.Basic
...
private theorem interIUnion (set : Set α) (sets : Nat → Set α) :
    Set.inter set (Set.iUnion sets) =
      Set.iUnion (fun index => Set.inter set (sets index)) := by
  apply Set.ext
  intro value
  constructor
  · rintro ⟨setMember, index, familyMember⟩
    exact ⟨index, setMember, familyMember⟩
  · rintro ⟨index, setMember, familyMember⟩
    exact ⟨setMember, index, familyMember⟩

private theorem interUniv (set : Set α) :
    Set.inter Set.univ set = set := by
  apply Set.ext
  intro value
...
public theorem isCaratheodory_iUnion_of_disjoint
  ...
  rw [splitUnion]
  unfold ENNReal.tsum
  rw [ENNReal.addComm, ENNReal.addISup]
  ...
  have bound := ENNReal.addLeAddLeft remainingBound
...
public theorem ofFunction_caratheodory
  ...
  have interBound : ENNReal.le (outer (Set.inter set region))
    ...
    ENNReal.leTrans (outer.mono_apply interCovered)
      (ENNReal.leTrans (outer.iUnion_apply_le _)
        (ENNReal.tsumLeTsum fun index =>
  ...
  have differenceBound : ENNReal.le (outer (Set.difference set region))
    ...
    ENNReal.leTrans (outer.mono_apply differenceCovered)
      (ENNReal.leTrans (outer.iUnion_apply_le _)
        (ENNReal.tsumLeTsum fun index =>
          OuterMeasure.ofFunction_le cost emptyCost _))
  have firstBound := ENNReal.addLeAdd interBound differenceBound
  have splitBound := ENNReal.tsumLeTsum fun index =>
    split (cover.sets index)
  rw [ENNReal.tsumAdd] at splitBound
  unfold coverCost
  exact ENNReal.leTrans firstBound splitBound
