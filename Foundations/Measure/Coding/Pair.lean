...
namespace Foundations.Measure.Coding.Pair
...
@[expose] public noncomputable def decode (input : Carrier) : UnitInterval × UnitInterval :=
  (Unit.value (fun index => Foundations.Real.Coding.digit (ENNReal.ofReal input) (2 * index)),
    Unit.value (fun index => Foundations.Real.Coding.digit (ENNReal.ofReal input) (2 * index + 1)))
...
public theorem decode_encode (point : UnitInterval × UnitInterval) :
  ...
  have even : (fun index => Foundations.Real.Coding.digit (ENNReal.ofReal (encode point)) (2 * index)) =
      Unit.bits point.1 := by
    funext index
    rw [represented, Foundations.Real.Coding.digit_encode, bits_even]
  have odd : (fun index => Foundations.Real.Coding.digit (ENNReal.ofReal (encode point)) (2 * index + 1)) =
      Unit.bits point.2 := by
    funext index
    rw [represented, Foundations.Real.Coding.digit_encode, bits_odd]
  unfold decode
...
public theorem decode_measurable :
  ...
  · exact Unit.value_measurable (fun index =>
      ofRealMeasurable.measurableMap (Coding.digit_measurable (2 * index)))
  · exact Unit.value_measurable (fun index =>
      ofRealMeasurable.measurableMap (Coding.digit_measurable (2 * index + 1)))

end Foundations.Measure.Coding.Pair
