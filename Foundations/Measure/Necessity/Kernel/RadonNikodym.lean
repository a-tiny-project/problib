...
public import Foundations.Measure.Necessity.Measurability
public import Foundations.Measure.Necessity.Density
public import Foundations.Measure.Kernel.RadonNikodym
...
namespace Foundations.Measure.Necessity.Kernel.RadonNikodym
...
public noncomputable def reference :
    Foundations.Measure.Kernel (Space.indiscrete Bool) (Space.discrete Bool) :=
  Foundations.Measure.Kernel.const (Space.indiscrete Bool)
...
public noncomputable def separateDerivative (input : Bool) :
  ...
  densityMeasurable := fun _ => True.intro
  reconstruct := by
...
public theorem density_not_jointlyMeasurable :
...
public theorem ordinary_absoluteContinuity_insufficient :
    ∃ kernel base : Foundations.Measure.Kernel (Space.discrete Unit) (Space.discrete Unit),
      Nonempty (Foundations.Measure.Kernel.IsSFinite kernel) ∧
        Nonempty (Foundations.Measure.Kernel.IsSFinite base) ∧
        (∀ input, Measure.AbsolutelyContinuous (kernel input) (base input)) ∧
        ¬Nonempty (Foundations.Measure.Kernel.RadonNikodymDerivative kernel base) := by
  let kernel := Foundations.Measure.Kernel.const (Space.discrete Unit)
    (Measure.dirac (Space.discrete Unit) ())
  let base := Foundations.Measure.Kernel.const (Space.discrete Unit) Density.reference
  refine ⟨kernel, base,
    ⟨Foundations.Measure.Kernel.IsSFinite.const _
      (Measure.SFinite.ofFinite (Measure.IsFinite.dirac _ _))⟩,
    ⟨Foundations.Measure.Kernel.IsSFinite.const _ Density.reference_sFinite⟩,
...
public noncomputable def empty_source_derivative {β : Type} (target : Space β)
    (kernel base : Foundations.Measure.Kernel (Space.discrete Empty) target) :
    Foundations.Measure.Kernel.RadonNikodymDerivative kernel base :=
...
public theorem infinite_reference_does_not_ensure_ae_uniqueness :
    ∃ kernel : Foundations.Measure.Kernel (Space.discrete Unit) (Space.discrete Unit),
      Nonempty (Foundations.Measure.Kernel.IsSFinite kernel) ∧
        ∃ left right : Foundations.Measure.Kernel.RadonNikodymDerivative kernel kernel,
          ¬(∀ input, (kernel input).AEEq (left.density input) (right.density input)) := by
  let kernel := Foundations.Measure.Kernel.const (Space.discrete Unit) Density.reference
  let left : Measure.RadonNikodymDerivative Density.reference Density.reference := {
    density := fun _ => ENNReal.one
    densityMeasurable := ENNRealMeasurable.constant _ _
    reconstruct := (Density.reference_withDensity_nonzero _ ENNReal.oneNeZero).symm
  }
  ...
  refine ⟨kernel, ⟨Foundations.Measure.Kernel.IsSFinite.const _ Density.reference_sFinite⟩,
    Foundations.Measure.Kernel.RadonNikodymDerivative.const left _,
    Foundations.Measure.Kernel.RadonNikodymDerivative.const right _, ?_⟩
  intro equal
...
public noncomputable def selected_probability_density :
    Foundations.Measure.Kernel.RadonNikodymDerivative reference reference :=
...
public theorem standardBorel_does_not_remove_zeroInfinity_premise :
    Nonempty (StandardBorel (Space.discrete Unit)) ∧
      ∃ kernel base : Foundations.Measure.Kernel (Space.discrete Unit) (Space.discrete Unit),
        Nonempty (Foundations.Measure.Kernel.IsSFinite kernel) ∧
          Nonempty (Foundations.Measure.Kernel.IsSFinite base) ∧
          (∀ input, Measure.AbsolutelyContinuous (kernel input) (base input)) ∧
          ¬Nonempty (Foundations.Measure.Kernel.RadonNikodymDerivative kernel base) :=
  ⟨⟨StandardBorel.unit⟩, ordinary_absoluteContinuity_insufficient⟩
...
public noncomputable def standardBorel_empty_source_derivative {β : Type v}
    ...
    (kernel base : Foundations.Measure.Kernel (Space.discrete Empty) target) :
    Foundations.Measure.Kernel.RadonNikodymDerivative kernel base :=
  Foundations.Measure.Kernel.RadonNikodymDerivative.ofStandardBorel presentation
    (Foundations.Measure.Kernel.IsSFinite.ofFiniteFibers (fun input => Empty.elim input))
    (Foundations.Measure.Kernel.IsSFinite.ofFiniteFibers (fun input => Empty.elim input))
    (fun input => Empty.elim input)
...
public noncomputable def standardBorel_empty_target_derivative
    ...
    Foundations.Measure.Kernel.RadonNikodymDerivative
      (Foundations.Measure.Kernel.zero source target) (Foundations.Measure.Kernel.zero source target) :=
  Foundations.Measure.Kernel.RadonNikodymDerivative.ofStandardBorel (StandardBorel.ofEmpty empty)
    (Foundations.Measure.Kernel.IsSFinite.zero source target)
    (Foundations.Measure.Kernel.IsSFinite.zero source target)
...
public theorem densityRatio_requires_factorable_infinite_mass :
  ...
  rw [ENNReal.densityRatio,
    if_neg (show ENNReal.top ≠ ENNReal.zero from ENNReal.topNeFinite NNReal.zero), if_pos rfl,
    if_neg ENNReal.oneNeZero, ENNReal.mulOne]
  exact ENNReal.topNeFinite NNReal.one

end Foundations.Measure.Necessity.Kernel.RadonNikodym
