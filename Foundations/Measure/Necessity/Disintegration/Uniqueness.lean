...
namespace Foundations.Measure.Necessity.Disintegration.Uniqueness
...
theorem weights_sum : ENNReal.add firstWeight secondWeight = ENNReal.one := by
  ...
  simp only [sum, NNReal.ofRatOne]
  rfl

theorem first_nonzero : firstWeight ≠ ENNReal.zero := by
  have positive : ENNReal.lt ENNReal.zero firstWeight := by
    rw [firstWeight, ← ENNReal.ofRatZero]
    exact (ENNReal.ofRatLtIff _ _ _ _).mpr first_positive_rat
  intro equal
...
theorem second_nonzero : secondWeight ≠ ENNReal.zero := by
  have positive : ENNReal.lt ENNReal.zero secondWeight := by
    rw [secondWeight, ← ENNReal.ofRatZero]
    exact (ENNReal.ofRatLtIff _ _ _ _).mpr second_positive_rat
  intro equal
  rw [equal] at positive
  exact positive.2 positive.1
...
theorem weights_different : firstWeight ≠ secondWeight := by
  have less : ENNReal.lt firstWeight secondWeight :=
    (ENNReal.ofRatLtIff _ _ _ _).mpr (by
...
theorem distribution_probability (first second : ENNReal)
  ...
  constructor
  simpa only [distribution_apply, Set.univ, if_true, ENNReal.mulOne] using normalized
...
theorem conditional_finite (first second : ENNReal)
  ...
  Kernel.IsFinite.const _ (distribution_probability first second normalized).toFinite
...
theorem products_equal : product firstWeight secondWeight weights_sum =
    product secondWeight firstWeight ((ENNReal.addComm _ _).trans weights_sum) := by
  apply Measure.ext
  ...
  by_cases first : set (false, ()) <;> by_cases second : set (true, ()) <;>
    simp only [distribution_apply, first, second, if_true, if_false,
      ENNReal.mulOne, ENNReal.mulZero, ENNReal.addZero, ENNReal.zeroAdd,
      ENNReal.zeroMul, ENNReal.mulTopOfNeZero first_nonzero,
      ENNReal.mulTopOfNeZero second_nonzero, ENNReal.addComm secondWeight firstWeight]
...
theorem product_univ (first second : ENNReal)
  ...
  rw [(distribution_probability first second normalized).univ_eq_one, ENNReal.oneMul]
...
noncomputable def secondDisintegration :
  ...
  conditionalSFinite := (conditional_finite _ _ ((ENNReal.addComm _ _).trans weights_sum)).toSFinite
  reconstruction := by
    rw [Measure.Disintegrates, marginal_eq]
...
theorem conditional_different (input : Unit) :
  ...
  simp only [distribution_apply, Bool.true_eq_false, if_true, if_false,
    ENNReal.mulOne, ENNReal.mulZero, ENNReal.addZero] at masses
...
theorem both_probability (input : Unit) :
  ...
  ⟨distribution_probability _ _ weights_sum,
    distribution_probability _ _ ((ENNReal.addComm _ _).trans weights_sum)⟩
...
theorem sFinite_probability_conditionals_not_unique :
...
end Foundations.Measure.Necessity.Disintegration.Uniqueness
