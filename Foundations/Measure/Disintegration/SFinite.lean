...
public import Foundations.Measure.Disintegration.Compatibility
public import Foundations.Measure.Disintegration.StandardBorel
...
public theorem ofZeroInfinity_isProbability_ae
  ...
  exact AbsolutelyContinuous.ae (AbsolutelyContinuous.map reference.targetContinuous Prod.snd
...
public theorem ofSFinite_univ_le_one (joint : Measure (Space.product source parameter))
    ...
    ENNReal.le ((ofSFinite joint presentation finite jointTop marginalTop compatible).conditional input Set.univ)
      ENNReal.one := by
  change ENNReal.le (Kernel.piecewise marginalTop.set marginalTop.zeroInfinity.measurable
...
public theorem ofSFinite_isProbability_ae (joint : Measure (Space.product source parameter))
  ...
  · apply isProbability_ae
    rw [secondMarginal_restrict_second joint (parameter.complement marginalTop.zeroInfinity.measurable)]
    exact marginalTop.sigmaFinite_complement (secondMarginalSFinite finite)
