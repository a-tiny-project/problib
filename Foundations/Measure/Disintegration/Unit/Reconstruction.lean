...
public import Foundations.Measure.Disintegration.Unit.Bounds
public import Foundations.Measure.Additive.Marginal.Uniqueness
...
private theorem conditional_initial_le_total
  ...
  rw [lintegral_const, ENNReal.oneMul, restrict_apply_univ] at bound
  exact bound
...
public theorem conditional_initial_of_finite
  ...
  · rw [initialMarginal_apply joint point measurable,
      ← slice_apply joint region (unitInitialMeasurable point)]
    refine le_measure_unitInitial_of_rightDense (slice_finite joint measurable regionFinite)
    ...
    · intro index active
      rw [slice_apply joint region (unitInitialMeasurable _),
...
private theorem reconstructed_rectangle_univ
  ...
  rw [normalized, lintegral_const, ENNReal.oneMul, restrict_apply_univ]
...
public theorem conditional_reconstruct
  ...
  have equal : slice reconstructed second = slice joint second := by
    apply finiteMeasure_ext_unitInitial reconstructedFinite (slice_finite joint secondMeasurable secondFinite)
    intro point
    rw [slice_apply reconstructed second (unitInitialMeasurable point),
      slice_apply joint second (unitInitialMeasurable point)]
    ...
    rw [reverseSemiproduct_apply_product _ _ _ (unitInitialMeasurable point) secondMeasurable,
...
public theorem conditional_initial
  ...
  rw [reverseSemiproduct_apply_product _ _ _ (unitInitialMeasurable point) measurable] at equal
  exact equal
