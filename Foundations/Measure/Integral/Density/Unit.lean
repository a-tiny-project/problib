...
public theorem IsDensity.le_of_unit {density : alpha → Real.UnitInterval}
  ...
  have bound : reference.AE (fun value =>
    ...
    ae_of_forall (fun value => (ENNReal.ofRealLeIffLeToReal
...
public theorem IsDensity.unit_ae_le_iff {left right : Measure space}
  ...
  have firstExtended : ENNRealMeasurable space
      (fun value => ENNReal.ofReal (first value).val) := Real.ofRealMeasurable.comp
    (MeasurableMap.comp Real.unitInclusionMeasurable firstMeasurable)
  have secondExtended : ENNRealMeasurable space
      (fun value => ENNReal.ofReal (second value).val) := Real.ofRealMeasurable.comp
    (MeasurableMap.comp Real.unitInclusionMeasurable secondMeasurable)
  ...
  exact Iff.trans
    ⟨fun holds => holds.mono (fun value included =>
        (ENNReal.ofRealLeOfRealIff (first value).property.1 (second value).property.1).mpr included),
      fun holds => holds.mono (fun value included =>
        (ENNReal.ofRealLeOfRealIff (first value).property.1 (second value).property.1).mp included)⟩
    comparison.symm
