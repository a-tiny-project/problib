...
private theorem nullSeparatedSet {left right : alpha → ENNReal}
  ...
  have regionFinite : ENNReal.Finite (measure region) := by
    have finiteRestriction := (finite.restrict region).univFinite
  ...
  intro nonzero
  have cancelled := (ENNReal.mulLeMulRightIff regionFinite
    (ENNReal.zeroLtIffNeZero.mpr nonzero)).mp
    (ENNReal.leTrans lowerBound (ENNReal.leTrans comparison upperBound))
...
public theorem ae_le_of_withDensity_le_finite {left right : alpha → ENNReal}
  ...
  have nullRegions : ∀ lower upper, measure.NullSet (regions lower upper) := by
    ...
    by_cases separated :
        ENNReal.lt (ENNReal.rationalBasis lower) (ENNReal.rationalBasis upper)
    · exact (nullSeparatedSet finite leftMeasurable rightMeasurable
  ...
  have less : ENNReal.lt (right value) (left value) :=
    ⟨(ENNReal.leTotal (left value) (right value)).resolve_left notIncluded,
      notIncluded⟩
  rcases ENNReal.existsRationalBasisBetween less with
    ⟨lower, rightLower, lowerLeft⟩
  rcases ENNReal.existsRationalBasisBetween lowerLeft with
