...
public theorem withDensity_finite_of_bounded {reference : Measure space}
  ...
  exact ENNReal.finiteOfLe upper (ENNReal.mulFinite boundFinite regionFinite)
...
public noncomputable def withDensity_sigmaFinite_restrict_ne_top {reference : Measure space}
  ...
  · intro index
    ...
    apply withDensity_finite_of_bounded density
      ...
      (finite.apply _) (ENNReal.rationalBasisFinite index)
    intro value member
  ...
  · apply Set.ext
    ...
    · intro _
      ...
      · have below : ENNReal.lt (density value) ENNReal.top :=
          ⟨ENNReal.leTop _, fun reverse => infinite (ENNReal.topLeIff.mp reverse)⟩
        rcases ENNReal.existsRationalBasisBetween below with ⟨index, included, _⟩
