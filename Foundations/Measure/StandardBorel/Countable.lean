...
private theorem reciprocalSuccessor_mem (index : Nat) :
    Real.unitSet
  ...
  · rw [← Dedekind.ofRatZero]
    exact (Dedekind.ofRatLeIff _ _).mpr
      (Rat.le_of_lt (Rat.inv_pos.mpr positive))
  · rw [← Dedekind.ofRatOne]
    apply (Dedekind.ofRatLeIff _ _).mpr
...
public theorem naturalCode_injective : Function.Injective naturalCode := by
  ...
  have rational := Dedekind.ofRatInjective
...
private theorem singleton_measurable (point : Real.UnitInterval) :
  ...
  have equal : (fun value : Real.UnitInterval => value = point) =
    ...
    · intro member
      subst value
      exact ⟨Dedekind.leRefl _, Dedekind.leRefl _⟩
    · intro member
      exact Subtype.ext (Dedekind.leAntisymm member.2 member.1)
  rw [equal]
  exact Real.unitInclusionMeasurable (Real.measurable_Icc _ _)
...
public noncomputable def ofNatInjection {alpha : Type u}
  ...
  refine {
    range := range
    rangeMeasurable := naturalCode_image_measurable code Set.univ
    equivalence := {
      forward := forward
      inverse := inverse
      inverse_forward := inverseForward
      forward_inverse := fun point => Subtype.ext (inverseCode point)
      forward_measurable := MeasurableMap.fromDiscrete subtypeSpace forward
