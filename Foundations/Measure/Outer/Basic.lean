...
private theorem iUnionPairSets {α : Type u} (left right : Set α) :
...
private theorem pairValues_eq_addSingles (left right : ENNReal) :
...
private theorem tsumPairValues (left right : ENNReal) :
    ENNReal.tsum (pairValues left right) = ENNReal.add left right := by
  rw [pairValues_eq_addSingles, ENNReal.tsumAdd,
    ENNReal.tsumSingle, ENNReal.tsumSingle]
...
theorem union_apply_le {α : Type u} (outer : OuterMeasure α)
  ...
  rw [iUnionPairSets, valuesEqual, tsumPairValues] at included
  exact included
