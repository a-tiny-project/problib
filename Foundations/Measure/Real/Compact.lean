...
public theorem prefixCoverMonotone
...
private theorem ltOfLtOfLe {left middle right : Carrier}
    (leftMiddle : Dedekind.lt left middle)
    (middleRight : Dedekind.le middle right) :
    Dedekind.lt left right := by
  refine ⟨Dedekind.leTrans leftMiddle.1 middleRight, ?_⟩
  intro rightLeft
  exact leftMiddle.2 (Dedekind.leTrans middleRight rightLeft)

private theorem ltOfLeOfLt {left middle right : Carrier}
    (leftMiddle : Dedekind.le left middle)
    (middleRight : Dedekind.lt middle right) :
    Dedekind.lt left right := by
  refine ⟨Dedekind.leTrans leftMiddle middleRight.1, ?_⟩
  intro rightLeft
  exact middleRight.2 (Dedekind.leTrans rightLeft leftMiddle)

private theorem leOfNotGe {left right : Carrier}
    (notGe : ¬Dedekind.le right left) : Dedekind.le left right := by
  rcases Dedekind.leTotal left right with included | reversed
  · exact included
  · exact False.elim (notGe reversed)

private theorem pointAboveNonUpper
    {reachable : Carrier → Prop} {least lower : Carrier}
    (leastMinimal : ∀ upper,
      Foundations.Real.IsUpperBound Dedekind.le reachable upper →
        Dedekind.le least upper)
    (lowerLeast : Dedekind.lt lower least) :
    ∃ point, reachable point ∧ Dedekind.lt lower point := by
  have notUpper :
      ¬Foundations.Real.IsUpperBound Dedekind.le reachable lower := by
    intro lowerUpper
    exact lowerLeast.2 (leastMinimal lower lowerUpper)
  unfold Foundations.Real.IsUpperBound at notUpper
  ...
  exact ⟨point, pointReachable,
    ⟨leOfNotGe notPointLower, notPointLower⟩⟩

private theorem combinePrefixCover
    (intervals : Nat → Carrier × Carrier)
  ...
  · exact prefixCoverMonotone intervals (Nat.le_max_left _ _)
      (oldCover ⟨valueClosed.1, beforePoint⟩)
  · have pointValue : Dedekind.lt point value :=
      ⟨leOfNotGe beforePoint, beforePoint⟩
    have intervalMember : openInterval (intervals intervalIndex) value :=
      ⟨ltOfLtOfLe intervalLeftPoint pointValue.1,
        ltOfLeOfLt valueClosed.2 targetIntervalRight⟩
...
public theorem existsPrefixCover
    (intervals : Nat → Carrier × Carrier)
  ...
  have leftCovered : (Set.iUnion family) left :=
    covers ⟨Dedekind.leRefl left, ordered⟩
  rcases leftCovered with ⟨leftIndex, leftInterval⟩
  have leftReachable : reachable left := by
    refine ⟨Dedekind.leRefl left, ordered, leftIndex + 1, ?_⟩
    intro value valueClosed
    have valueLeft : value = left :=
      Dedekind.leAntisymm valueClosed.2 valueClosed.1
    subst value
    exact Set.subset_prefixUnion_succ family leftIndex leftInterval
  have reachableNonempty : ∃ endpoint, reachable endpoint :=
    ⟨left, leftReachable⟩
  have rightUpper :
      Foundations.Real.IsUpperBound Dedekind.le reachable right :=
    fun _ member => member.2.1
  have reachableBounded :
      ∃ upper, Foundations.Real.IsUpperBound Dedekind.le reachable upper :=
    ⟨right, rightUpper⟩
  rcases Dedekind.existsLub reachable reachableNonempty reachableBounded with
  ...
  have rightLeast : Dedekind.le right least := by
    ...
    rcases pointAboveNonUpper leastMinimal intervalAtLeast.1 with
    ...
    obtain ⟨target, leastTarget, targetRight, targetIntervalRight⟩ :
      ...
      rcases Dedekind.leTotal right (intervals intervalIndex).2 with
        rightBeforeInterval | intervalBeforeRight
      · rcases Dedekind.existsRationalBetween leastRightStrict with
          ⟨rational, leastTarget, targetBeforeRight⟩
        exact ⟨Dedekind.selection.ofRat rational, leastTarget,
          targetBeforeRight.1,
          ltOfLtOfLe targetBeforeRight rightBeforeInterval⟩
      · rcases Dedekind.existsRationalBetween intervalAtLeast.2 with
          ⟨rational, leastTarget, targetBeforeInterval⟩
        exact ⟨Dedekind.selection.ofRat rational, leastTarget,
          Dedekind.leTrans targetBeforeInterval.1 intervalBeforeRight,
          targetBeforeInterval⟩
    rcases pointReachable with ⟨_, _, oldBound, oldCover⟩
    rcases combinePrefixCover intervals oldCover intervalLeftPoint
      targetIntervalRight with ⟨bound, targetCover⟩
    have targetReachable : reachable target :=
      ⟨ltOfLeOfLt leftLeast leastTarget |>.1, targetRight,
        bound, targetCover⟩
    exact leastTarget.2 (leastUpper target targetReachable)
  have leastRightEqual : least = right :=
    Dedekind.leAntisymm leastRight rightLeast
  have rightCovered : (Set.iUnion family) right :=
    covers ⟨ordered, Dedekind.leRefl right⟩
  ...
  rcases pointAboveNonUpper leastMinimal intervalLeftLeast with
    ⟨point, pointReachable, intervalLeftPoint⟩
  rcases pointReachable with ⟨_, _, oldBound, oldCover⟩
  exact combinePrefixCover intervals oldCover intervalLeftPoint intervalAtRight.2
