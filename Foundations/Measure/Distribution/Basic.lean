...
structure DistributionFunction where
  ...
  rightContinuous : ∀ point, Dedekind.lt point.val Dedekind.one →
...
theorem DistributionFunction.function_measurable (distribution : DistributionFunction) :
  ...
  unitMonotone_measurable (fun included => distribution.monotone included)
...
theorem DistributionFunction.rightContinuous_bounds (distribution : DistributionFunction)
  ...
  rcases distribution.rightContinuous point room upper upperBound with
    ⟨right, rightAbove, rightBelow⟩
  refine ⟨right, rightAbove, ?_⟩
  intro input above below
  exact ⟨ltOfLtOfLe lowerBound (distribution.monotone above),
    ltOfLeOfLt (distribution.monotone below) rightBelow⟩
