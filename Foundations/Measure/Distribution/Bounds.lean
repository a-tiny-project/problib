...
private theorem existsBoundValue (thresholds bounds : ι → UnitInterval)
    (point : UnitInterval) :
    ∃ value : UnitInterval,
...
private noncomputable def boundValue (thresholds bounds : ι → UnitInterval)
  ...
  Classical.choose (existsBoundValue thresholds bounds point)
...
private theorem boundValue_lower (thresholds bounds : ι → UnitInterval)
  ...
  (Classical.choose_spec (existsBoundValue thresholds bounds point)).1 index active
...
private theorem boundValue_greatest (thresholds bounds : ι → UnitInterval)
  ...
  (Classical.choose_spec (existsBoundValue thresholds bounds point)).2 lower bounded
...
private theorem boundValue_lt_iff (thresholds bounds : ι → UnitInterval)
  ...
  · intro below
    ...
    · apply Or.inr
      ...
      intro missing
      have upperNonnegative := Dedekind.leTrans
        (boundValue thresholds bounds point).property.1 below.1
      apply below.2
      apply boundValue_greatest thresholds bounds point
        ⟨upper, upperNonnegative, notLtIffLe.mp aboveOne⟩
      intro index active
      exact notLtIffLe.mp (fun strictlyBelow => missing ⟨index, active, strictlyBelow⟩)
  · intro witness
    rcases witness with aboveOne | ⟨index, active, below⟩
    · exact ltOfLeOfLt (boundValue thresholds bounds point).property.2 aboveOne
    · exact ltOfLeOfLt (boundValue_lower thresholds bounds point index active) below
...
noncomputable def ofUpperBounds (thresholds bounds : ι → UnitInterval) :
  ...
  monotone := by
    ...
    intro index active
    exact boundValue_lower thresholds bounds left index (ltOfLeOfLt included active)
  upper := by
    apply Subtype.ext
    apply Dedekind.leAntisymm (boundValue thresholds bounds unitOne).property.2
  ...
  rightContinuous := by
    ...
    · exact ⟨unitOne, room,
        ltOfLeOfLt (boundValue thresholds bounds unitOne).property.2 aboveOne⟩
    · rcases existsUnitRightBelow point (thresholds index).val room active with
...
theorem ofUpperBounds_mono (thresholds left right : ι → UnitInterval)
  ...
  intro index active
  exact Dedekind.leTrans (ofUpperBounds_le thresholds left point index active) (included index)
...
theorem ofUpperBounds_of_inactive (thresholds bounds : ι → UnitInterval)
  ...
  apply Subtype.ext
  apply Dedekind.leAntisymm ((ofUpperBounds thresholds bounds).function point).property.2
...
theorem ofUpperBounds_eq (distribution : DistributionFunction)
  ...
  have same : (ofUpperBounds thresholds
    ...
    · apply Classical.byContradiction
      intro missing
      have below := notLeIffLt.mp missing
      have room : Dedekind.lt point.val Dedekind.one := by
        refine ⟨point.property.2, ?_⟩
        intro reverse
        have equal : point = unitOne := Subtype.ext (Dedekind.leAntisymm point.property.2 reverse)
      ...
      rcases distribution.rightContinuous point room _ below with ⟨right, above, valueBelow⟩
      rcases dense point right above with ⟨index, active, included⟩
      exact valueBelow.2 (Dedekind.leTrans
