...
public import Foundations.Measure.Space.Algebra
public import Foundations.Measure.Set.Difference
...
public theorem setDistance_triangle (measure : Measure space) (first middle last : Set α) :
  ...
  ENNReal.leTrans (measure.mono (Set.symmDiff_triangle first middle last))
...
public theorem setDistance_union (measure : Measure space) (first second left right : Set α) :
  ...
  ENNReal.leTrans (measure.mono (Set.symmDiff_union first second left right))
    (measure.union_le _ _)
...
public theorem le_add_setDistance (measure : Measure space) (left right : Set α) :
  ...
  ENNReal.leTrans (measure.mono (Set.subset_union_symmDiff left right)) (measure.union_le _ _)
...
public theorem union {left right : Set α}
  ...
  refine ⟨index, ?_⟩
  rw [equal]
  have bound := ENNReal.leTrans (measure.setDistance_union left right _ _)
    (ENNReal.addLeAdd firstBound secondBound)
