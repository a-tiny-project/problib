...
public theorem exists_measurable_superset_lt (measure : Measure space)
  ...
  have measurable : ∀ index, space.Measurable (cover.sets index) := by
    ...
    exact costBelow.2 (ENNReal.leTrans (ENNReal.leTop upper) bound)
  ...
  exact ⟨ENNReal.leTrans unionBound costBelow.1,
    fun reverse => costBelow.2 (ENNReal.leTrans reverse unionBound)⟩
...
public theorem NullSet.exists_measurable_superset {measure : Measure space}
  ...
  apply Classical.byContradiction
  intro nonzero
  have totalBound := ENNReal.leTrans
    (ENNReal.tsumLeTsum (fun index => ENNReal.leTrans
  ...
  have infinite : ENNReal.tsum (fun _ : Nat => measure (Set.iInter supersets)) =
      ENNReal.top := ENNReal.tsumConstOfNeZero nonzero
