...
public import Foundations.Measure.Decomposition.ZeroInfinity.AbsoluteContinuity
public import Foundations.Measure.Integral.Density.Algebra
public import Foundations.Measure.Integral.Density.AlmostEverywhere
public import Foundations.Measure.Integral.Density.Zero
public import Foundations.Measure.Integral.Simple.Induction
...
public theorem IsZeroInfinitySet.withDensity {measure : Measure space} {region : Set alpha}
  ...
  · intro selection selectionMeasurable value
    ...
    · exact Or.inl (zero ▸ ENNReal.mulZero value)
    · rw [infinite]
      classical
      by_cases valueZero : value = ENNReal.zero
      · exact Or.inl (valueZero ▸ ENNReal.zeroMul ENNReal.top)
      · exact Or.inr (ENNReal.mulTopOfNeZero valueZero)
  · intro left right leftMeasurable rightMeasurable leftCase rightCase
    ...
    · rw [zero, ENNReal.zeroAdd]
      exact rightCase
    · exact Or.inr (infinite ▸ ENNReal.topAdd _)
  · intro functions functionsMeasurable functionsMonotone cases
    ...
    · apply Or.inl
      ...
      · exact ENNReal.iSupLe (fun index => allZero index ▸ ENNReal.leRefl _)
      · exact ENNReal.zeroLe _
    · apply Or.inr
      apply ENNReal.topLeIff.mp
      ...
      · exact zero
      · have bound := ENNReal.leISup (fun index =>
