...
public theorem ae_levelDensity_eq (measure : Measure space) (regions : Nat → Set α)
  ...
  have upper : ENNReal.le (levelDensity regions value) (density value) := by
    ...
    · simpa only [ennrealIndicator, ennrealPiecewise, if_neg member] using ENNReal.zeroLe (density value)
  apply ENNReal.leAntisymm upper
  apply Classical.byContradiction
  intro notIncluded
  rcases ENNReal.existsRationalBasisBetween ⟨upper, notIncluded⟩ with ⟨index, above, below⟩
  have member : regions index value := by
    apply Classical.byContradiction
    intro absent
    exact below.2 ((bounded index).2 absent)
  have lower := ENNReal.leISup (fun index =>
