...
public import Foundations.Measure.Decomposition.Hahn
public import Foundations.Measure.Additive.Subtract
public import Foundations.Measure.Integral.Density.Domination
public import Foundations.Measure.Extended.Algebra
...
public theorem exists_radonNikodymDerivative_of_finite {target reference : Measure space}
  ...
  have densityFinite := bound.toFinite targetFinite
  ...
  have reconstruct : target = reference.withDensity density := by
    ...
    have positive : ENNReal.lt ENNReal.zero (remainder Set.univ) := by
      apply ENNReal.zeroLtIffNeZero.mpr
      intro zero
    ...
    have nonzero : (reference.withDensity increment) Set.univ ≠ ENNReal.zero := by
      ...
      intro zero
      rcases ENNReal.mulEqZeroIff.mp zero with factorZero | regionZero
      · exact ENNReal.zeroLtIffNeZero.mp factorPositive factorZero
      · exact ENNReal.zeroLtIffNeZero.mp regionPositive regionZero
    apply nonzero
    apply ENNReal.addLeftCancelOfFinite
      (factor := (reference.withDensity density) Set.univ) densityFinite.univFinite
    rw [ENNReal.addZero]
    apply ENNReal.leAntisymm upper
    have lower := ENNReal.addLeAddLeft
      (ENNReal.zeroLe ((reference.withDensity increment) Set.univ))
      ((reference.withDensity density) Set.univ)
    rw [ENNReal.addZero] at lower
    exact lower
  exact ⟨{ density := density, densityMeasurable := bound.measurable, reconstruct := reconstruct }⟩
