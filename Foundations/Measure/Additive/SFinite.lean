...
measure. -/
public theorem FiniteCover.restrictFinite {measure : Measure space}
...
public def of_le {left right : Measure space} (finite : SigmaFinite right)
  ...
  finite := fun index => ENNReal.finiteOfLe
...
private theorem prefixFinite {measure : Measure space}
  ...
  | succ count induction =>
      apply ENNReal.finiteOfLe (measure.union_le
        (Set.prefixUnion cover.sets count) (cover.sets count))
      exact ENNReal.addFinite induction (cover.finite count)
...
public def ofFinite {measure : Measure space}
  ...
  monotone := by
    ...
    exact True.intro
  finite := fun _ => finite.univFinite
  cover := by
    apply Set.ext
    intro value
...
public noncomputable def SigmaFinite.toSFinite {measure : Measure space}
  ...
  refine {
    ...
    finite := cover.toFiniteCover.restrictFinite
    sum_eq := ?_
  }
  calc
