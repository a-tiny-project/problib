...
import all Foundations.Real.Construction.Dedekind.Neg
...
theorem addNeg (cut : Cut) : cut + (-cut) = 0 := by
  apply Cut.ext
  intro value
  constructor
  · rintro ⟨inside, insideMember, negated,
    ...
    have insideOutside : inside ≤ outside :=
      cut.leOfMemOfNotMem insideMember outsideAbsent
    ...
    have sumNonpositive : inside + -outside ≤ 0 := by
      ...
      exact translated
    exact Rational.ltOfLtOfLe valueLess sumNonpositive
  · intro zeroMember
    ...
    rcases cut.existsInnerOuterGapLt negValuePositive with
...
theorem addLeAddRight {left right : Cut} (included : left ≤ right)
