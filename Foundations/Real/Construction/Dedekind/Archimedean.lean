...
theorem existsInnerOuterGapLt (cut : Cut) {epsilon : Rat}
  ...
  have stridePositive : 0 < stride := Rational.halfPos epsilonPositive
  have strideLess : stride < epsilon := Rational.halfLt epsilonPositive
  ...
  have allStepsMember :
    ...
    | succ index inductionHypothesis =>
        exact stepMember (Rational.step start stride index)
          inductionHypothesis
  rcases Rational.existsStepGt start boundary stride stridePositive with
    ⟨index, boundaryStep⟩
