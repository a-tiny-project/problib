...
def add (left right : Cut) : Cut where
  ...
  proper := by
    ...
    have impossible := Rational.ltTrans firstLess secondLess
  ...
  noGreatest := by
    intro value
    rintro ⟨leftValue, leftMember, rightValue, rightMember, equal⟩
    rcases left.noGreatest leftMember with
...
theorem addComm (left right : Cut) : left + right = right + left := by
  apply Cut.ext
  intro value
  constructor
  · rintro ⟨leftValue, leftMember, rightValue, rightMember, equal⟩
...
theorem addAssoc (left middle right : Cut) :
...
theorem addZero (value : Cut) : value + 0 = value := by
  apply Cut.ext
  intro rational
  constructor
  · rintro ⟨leftValue, leftMember, rightValue, rightMember, equal⟩
