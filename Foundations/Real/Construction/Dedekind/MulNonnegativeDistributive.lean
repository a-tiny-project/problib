...
import all Foundations.Real.Construction.Dedekind.MulNonnegative
...
private theorem addNonnegative {left right : Cut}
    (leftNonnegative : 0 ≤ left) (rightNonnegative : 0 ≤ right) :
    0 ≤ left + right := by
  have shifted := Cut.addLeAddRight leftNonnegative right
  rw [Cut.addComm 0 right, Cut.addZero] at shifted
  exact leTrans rightNonnegative shifted

private theorem existsGreaterMemberOfLtMul
    {left right : Cut}
    {leftNonnegative : 0 ≤ left} {rightNonnegative : 0 ≤ right}
  ...
  · rcases Rational.existsBetween targetBound with
  ...
  · have rightNonpositive : rightValue ≤ 0 := Rat.not_lt.mp rightPositive
    have productNonpositive : leftValue * rightValue ≤ 0 := by
      ...
      exact included
    have targetNegative :=
      Rational.ltOfLtOfLe targetBound productNonpositive
    rcases Rational.existsBetween targetNegative with
      ⟨greater, targetGreater, greaterNegative⟩
    exact ⟨greater, Or.inl greaterNegative, targetGreater⟩

private theorem memberAddOfLtSum {left right : Cut}
    {leftValue rightValue target : Rat}
...
private theorem memberAddOfLeftSlack {left right : Cut}
...
private theorem memberAddOfRightSlack {left right : Cut}
...
theorem mulNonnegativeAddLeft (factor left right : Cut)
    (factorNonnegative : 0 ≤ factor)
    (leftNonnegative : 0 ≤ left) (rightNonnegative : 0 ≤ right) :
    mulNonnegative factor (left + right)
        factorNonnegative (addNonnegative leftNonnegative rightNonnegative) =
  ...
  · intro targetMember
    have leftProductNonnegative :=
      mulNonnegativeNonnegative factor left
        factorNonnegative leftNonnegative
    have rightProductNonnegative :=
      mulNonnegativeNonnegative factor right
        factorNonnegative rightNonnegative
    have sumNonnegative := addNonnegative
    ...
    · rcases positiveProduct with
      ...
      rcases existsGreaterMemberOfLtMul
    ...
    · have targetNonnegative : 0 ≤ target := Rat.not_lt.mp targetNegative
      ...
      · rcases rightProductMember with
        ...
        · have sumLessRight :
            ...
            exact less
          have sumNegative := Rational.ltTrans
        ...
        · rcases rightPositiveProduct with
          ...
          have targetBound : target < factorValue * rightValue :=
            Rational.ltTrans (by rw [targetEqual]; exact sumLessRight)
              rightBound
          ...
          exact Or.inr ⟨factorValue, factorMember, factorPositive,
            sumValue,
            memberAddOfRightSlack leftNonnegative
      ...
      · rcases leftPositiveProduct with
        ...
        · have sumLessLeft :
          ...
          have targetBound : target < firstFactorValue * leftValue :=
            Rational.ltTrans (by rw [targetEqual]; exact sumLessLeft)
              leftBound
          rcases Rational.existsPosLtOfLtMulLeft
          ...
          exact Or.inr ⟨firstFactorValue, firstFactorMember,
            ...
            memberAddOfLeftSlack rightNonnegative
        ...
        · rcases rightPositiveProduct with
          ...
          have leftProductBound :
            ...
            exact Rational.ltTrans leftBound factorBound
          have rightProductBound :
            ...
            exact Rational.ltTrans rightBound factorBound
          have sumBound :
              leftProductValue + rightProductValue <
                factorValue * leftValue + factorValue * rightValue :=
            Rational.addLtAdd leftProductBound rightProductBound
          ...
          exact Or.inr ⟨factorValue, factorMember, factorPositive,
            sumValue,
            memberAddOfLtSum leftMember rightMember sumBound,
            sumPositive, productBound⟩
