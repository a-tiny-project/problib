...
def neg (cut : Cut) : Cut where
  ...
  nonempty := by
    ...
    exact ⟨-outside - 1, outside, absent, Rational.sub_one_lt (-outside)⟩
  proper := by
  ...
  downward := by
    ...
    exact ⟨outside, absent, Rational.lt_trans smallerValue valueOutside⟩
  no_greatest := by
    intro value
    rintro ⟨outside, absent, valueOutside⟩
    rcases Rational.exists_between valueOutside with
