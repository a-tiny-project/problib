...
namespace Problib.Real.Construction.Rational

theorem lt_trans {left middle right : Rat}
...
theorem lt_of_le_of_lt {left middle right : Rat}
    (leftMiddle : left ≤ middle) (middleRight : middle < right) :
    left < right := by
  have leftRight : left ≤ right :=
...
theorem lt_of_lt_of_le {left middle right : Rat}
    (leftMiddle : left < middle) (middleRight : middle ≤ right) :
    left < right := by
  have leftRight : left ≤ right :=
...
theorem add_lt_add {firstLeft firstRight secondLeft secondRight : Rat}
  ...
  exact lt_trans firstStep secondStep

theorem sub_one_lt (value : Rat) : value - 1 < value := by
...
private theorem two_eq_one_add_one : (2 : Rat) = 1 + 1 := by
  simp [Rat.add_def']
  rfl

private theorem mul_two (value : Rat) : value * 2 = value + value := by
  rw [two_eq_one_add_one, Rat.mul_add]
  simp

theorem exists_between {left right : Rat} (less : left < right) :
...
private theorem step_eq (start stride : Rat) (index : Nat) :
...
theorem exists_nat_gt (value : Rat) :
  ...
  · have castNonpositive : (integer : Rat) ≤ 0 :=
    ...
    exact ⟨0, lt_of_lt_of_le valueInteger castNonpositive⟩
...
theorem exists_step_gt (start boundary stride : Rat)
    (stridePositive : 0 < stride) :
    ∃ index : Nat, boundary < step start stride index := by
  rcases exists_nat_gt ((boundary - start) / stride) with
    ⟨index, quotientBound⟩
  ...
  have reordered : boundary < (index : Rat) * stride + start :=
    (Rat.sub_lt_iff
      ...
      differenceBound
  refine ⟨index, ?_⟩
  rw [step_eq, Rat.add_comm]
  exact reordered

theorem half_pos {value : Rat} (positive : 0 < value) :
    0 < value / 2 := by
  have twoPositive : (0 : Rat) < 2 := by decide
...
theorem half_lt {value : Rat} (positive : 0 < value) :
    value / 2 < value := by
  have twoPositive : (0 : Rat) < 2 := by decide
...
theorem mul_le_mul_of_le_of_le_of_pos_of_pos {firstLeft firstRight secondLeft secondRight : Rat}
...
theorem mul_lt_of_lt_one_of_pos {left right : Rat}
...
theorem exists_pos_lt_one_mul_gt {lower upper : Rat}
    (lowerNonnegative : 0 ≤ lower) (lowerUpper : lower < upper) :
    ∃ factor, 0 < factor ∧ factor < 1 ∧ lower < factor * upper := by
  have upperPositive : 0 < upper := lt_of_le_of_lt lowerNonnegative lowerUpper
  have quotientLessOne : lower / upper < 1 := by
    apply (Rat.div_lt_iff upperPositive).mpr
    rw [Rat.one_mul]
    exact lowerUpper
  rcases exists_between quotientLessOne with ⟨factor, lowerFactor, factorOne⟩
  ...
  have factorPositive : 0 < factor :=
    lt_of_le_of_lt quotientNonnegative lowerFactor
...
theorem exists_pos_lt_of_lt_mul_left {lower factor upper : Rat}
  ...
  have quotientBound : lower / factor < upper := by
    ...
    rw [Rat.mul_comm]
    exact productBound
  rcases exists_between quotientBound with
  ...
  have smallerPositive : 0 < smaller :=
    lt_of_le_of_lt quotientNonnegative quotientSmaller
...
theorem exists_pos_lt_of_lt_mul_right {lower upper factor : Rat}
    (lowerNonnegative : 0 ≤ lower) (factorPositive : 0 < factor)
  ...
  have reordered : lower < factor * upper := by
    rw [Rat.mul_comm]
    exact productBound
  rcases exists_pos_lt_of_lt_mul_left lowerNonnegative factorPositive reordered with
...
end Problib.Real.Construction.Rational
