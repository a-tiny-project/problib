...
private theorem weight_add_budget (index : Nat) :
  ...
  have bound := NNReal.add_le_add_left
    (NNReal.half_le (NNReal.half (NNReal.dyadic NNReal.one (2 * index))))
    (NNReal.half (NNReal.dyadic NNReal.one (2 * index)))
  rw [NNReal.half_add_half] at bound
...
private theorem weight_not_le_budget (index : Nat) :
  ...
  have successor : 2 * (index + 1) = (2 * index + 1) + 1 := by omega
  have less := NNReal.half_lt (NNReal.dyadic_positive NNReal.one_positive (2 * index + 1))
...
private theorem partialSum_add_budget (bits : Nat → Bool) (start count : Nat) :
  ...
  | zero =>
      rw [partialSum, zero_add, Nat.add_zero]
      exact le_refl _
  | succ count induction =>
      rw [partialSum, add_assoc, Nat.add_succ]
      exact le_trans (add_le_add_left
        (le_trans (add_le_add_right (term_le_weight bits (start + count)) _)
...
private theorem tail_le_budget (bits : Nat → Bool) (start : Nat) :
  ...
  apply tsum_le
  intro count
  have bound := add_le_add_left (zero_le (budget (start + count)))
    (partialSum (fun index => term bits (start + index)) count)
  rw [add_zero] at bound
  exact le_trans bound (partialSum_add_budget bits start count)
...
public theorem encode_finite (bits : Nat → Bool) : Finite (encode bits) :=
  finite_of_le (encode_le_one bits) True.intro
...
public theorem prefix_finite (bits : Nat → Bool) (count : Nat) :
  ...
  finite_of_le (partialSum_le_tsum (term bits) count) (encode_finite bits)
...
public theorem threshold_iff (bits : Nat → Bool) (index : Nat) :
  ...
  · intro included
    ...
    intro missing
    have decomposition := partialSum_add_tail (term bits) (index + 1)
    rw [partialSum, term, if_neg missing, add_zero] at decomposition
    have bound : le (add (partialSum (term bits) index) (weight index))
        (add (partialSum (term bits) index) (budget (index + 1))) := by
      apply le_trans included
      rw [encode, ← decomposition]
      exact add_le_add_left (tail_le_budget bits (index + 1)) _
    exact weight_not_le_budget index
      (le_of_add_le_add_left_of_finite (prefix_finite bits index) bound)
  · intro present
    have bound := partialSum_le_tsum (term bits) (index + 1)
