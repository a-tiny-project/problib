...
private theorem massWeights_eq_integral_indicator {α : Type u} [DecidableEq α]
...
private theorem massWeights_zero_outside {α : Type u} [DecidableEq α]
    (point : α) (weights : List (α × NNRat))
...
theorem mass_eq_integral_indicator {α : Type u} [DecidableEq α]
  ...
  massWeights_eq_integral_indicator point measure.weights
...
theorem mass_zero_outside_support {α : Type u} [DecidableEq α]
  ...
  apply massWeights_zero_outside point measure.weights
  intro present
...
/-! ### Scaling and binding as equations

The monad laws above hold up to `≈ₘ`, which is the right strength for reasoning
about mass. A proof that reads a measure's rows, not its integrals, needs them
as equations between the weight lists, and they are: scaling and binding
rearrange rows without merging them, so each law holds row for row. -/

theorem scale_one {α : Type u} (measure : FiniteMeasure α) : scale 1 measure = measure := by
  obtain ⟨weights⟩ := measure
  induction weights with
  | nil => rfl
  | cons row rest inner =>
      rw [show scale 1 (⟨row :: rest⟩ : FiniteMeasure α)
          = ⟨(row.1, 1 * row.2) :: (scale 1 (⟨rest⟩ : FiniteMeasure α)).weights⟩
          from rfl, inner, NNRat.one_mul]

theorem scale_append {α : Type u} (factor : NNRat) (left right : List (α × NNRat)) :
    scale factor ⟨left ++ right⟩
      = (⟨(scale factor (⟨left⟩ : FiniteMeasure α)).weights
          ++ (scale factor (⟨right⟩ : FiniteMeasure α)).weights⟩ : FiniteMeasure α) := by
  induction left with
  | nil => rfl
  | cons row rest inner =>
      rw [List.cons_append,
        show scale factor (⟨row :: (rest ++ right)⟩ : FiniteMeasure α)
          = ⟨(row.1, factor * row.2) :: (scale factor (⟨rest ++ right⟩ : FiniteMeasure α)).weights⟩
          from rfl, inner,
        show scale factor (⟨row :: rest⟩ : FiniteMeasure α)
          = ⟨(row.1, factor * row.2) :: (scale factor (⟨rest⟩ : FiniteMeasure α)).weights⟩
          from rfl]
      rfl

theorem scale_scale {α : Type u} (outer inner : NNRat) (measure : FiniteMeasure α) :
    scale outer (scale inner measure) = scale (outer * inner) measure := by
  obtain ⟨weights⟩ := measure
  induction weights with
  | nil => rfl
  | cons row rest step =>
      rw [show scale inner (⟨row :: rest⟩ : FiniteMeasure α)
          = ⟨(row.1, inner * row.2) :: (scale inner (⟨rest⟩ : FiniteMeasure α)).weights⟩
          from rfl,
        show scale outer
            (⟨(row.1, inner * row.2) :: (scale inner (⟨rest⟩ : FiniteMeasure α)).weights⟩
              : FiniteMeasure α)
          = ⟨(row.1, outer * (inner * row.2))
              :: (scale outer (scale inner (⟨rest⟩ : FiniteMeasure α))).weights⟩ from rfl,
        step,
        show scale (outer * inner) (⟨row :: rest⟩ : FiniteMeasure α)
          = ⟨(row.1, outer * inner * row.2)
              :: (scale (outer * inner) (⟨rest⟩ : FiniteMeasure α)).weights⟩ from rfl,
        NNRat.mul_assoc]

theorem bind_dirac_left_eq {α : Type u} {β : Type v} (value : α)
    (kernel : α → FiniteMeasure β) : bind (dirac value) kernel = kernel value := by
  rw [show bind (dirac value) kernel
      = ⟨(scale 1 (kernel value)).weights ++ (bind (⟨[]⟩ : FiniteMeasure α) kernel).weights⟩
      from rfl,
    show (bind (⟨[]⟩ : FiniteMeasure α) kernel).weights = [] from rfl,
    List.append_nil, scale_one]

theorem bind_dirac_right_eq {α : Type u} (measure : FiniteMeasure α) :
    bind measure dirac = measure := by
  obtain ⟨weights⟩ := measure
  induction weights with
  | nil => rfl
  | cons row rest inner =>
      rw [show bind (⟨row :: rest⟩ : FiniteMeasure α) dirac
          = ⟨(scale row.2 (dirac row.1)).weights
              ++ (bind (⟨rest⟩ : FiniteMeasure α) dirac).weights⟩ from rfl, inner,
        show scale row.2 (dirac row.1) = ((⟨[(row.1, row.2 * 1)]⟩) : FiniteMeasure α)
          from rfl, NNRat.mul_one]
      rfl

theorem bind_append {α : Type u} {β : Type v} (left right : List (α × NNRat))
    (kernel : α → FiniteMeasure β) :
    bind ⟨left ++ right⟩ kernel
      = (⟨(bind (⟨left⟩ : FiniteMeasure α) kernel).weights
          ++ (bind (⟨right⟩ : FiniteMeasure α) kernel).weights⟩ : FiniteMeasure β) := by
  induction left with
  | nil => rfl
  | cons row rest inner =>
      rw [List.cons_append,
        show bind (⟨row :: (rest ++ right)⟩ : FiniteMeasure α) kernel
          = ⟨(scale row.2 (kernel row.1)).weights
              ++ (bind (⟨rest ++ right⟩ : FiniteMeasure α) kernel).weights⟩ from rfl, inner,
        show bind (⟨row :: rest⟩ : FiniteMeasure α) kernel
          = ⟨(scale row.2 (kernel row.1)).weights
              ++ (bind (⟨rest⟩ : FiniteMeasure α) kernel).weights⟩ from rfl]
      simp only [List.append_assoc]

theorem bind_scale {α : Type u} {β : Type v} (factor : NNRat) (measure : FiniteMeasure α)
    (kernel : α → FiniteMeasure β) :
    bind (scale factor measure) kernel = scale factor (bind measure kernel) := by
  obtain ⟨weights⟩ := measure
  induction weights with
  | nil => rfl
  | cons row rest inner =>
      rw [show scale factor (⟨row :: rest⟩ : FiniteMeasure α)
          = ⟨(row.1, factor * row.2) :: (scale factor (⟨rest⟩ : FiniteMeasure α)).weights⟩
          from rfl,
        show bind
            (⟨(row.1, factor * row.2) :: (scale factor (⟨rest⟩ : FiniteMeasure α)).weights⟩
              : FiniteMeasure α) kernel
          = ⟨(scale (factor * row.2) (kernel row.1)).weights
              ++ (bind (scale factor (⟨rest⟩ : FiniteMeasure α)) kernel).weights⟩
          from rfl, inner,
        show bind (⟨row :: rest⟩ : FiniteMeasure α) kernel
          = ⟨(scale row.2 (kernel row.1)).weights
              ++ (bind (⟨rest⟩ : FiniteMeasure α) kernel).weights⟩ from rfl,
        scale_append, ← scale_scale]

theorem bind_assoc_eq {α : Type u} {β : Type v} {γ : Type w} (measure : FiniteMeasure α)
    (first : α → FiniteMeasure β) (second : β → FiniteMeasure γ) :
    bind (bind measure first) second = bind measure fun value => bind (first value) second := by
  obtain ⟨weights⟩ := measure
  induction weights with
  | nil => rfl
  | cons row rest inner =>
      rw [show bind (⟨row :: rest⟩ : FiniteMeasure α) first
          = ⟨(scale row.2 (first row.1)).weights
              ++ (bind (⟨rest⟩ : FiniteMeasure α) first).weights⟩ from rfl,
        bind_append, bind_scale, inner,
        show bind (⟨row :: rest⟩ : FiniteMeasure α) (fun value => bind (first value) second)
          = ⟨(scale row.2 (bind (first row.1) second)).weights
              ++ (bind (⟨rest⟩ : FiniteMeasure α)
                  fun value => bind (first value) second).weights⟩ from rfl]

private theorem mapWeights_append {α : Type u} {β : Type v} (mapValue : α → β)
    (left right : List (α × NNRat)) :
    mapWeights mapValue (left ++ right) = mapWeights mapValue left ++ mapWeights mapValue right := by
  induction left with
  | nil => rfl
  | cons row rest inner => simp only [List.cons_append, mapWeights, inner]

private theorem mapWeights_scaleWeights {α : Type u} {β : Type v} (mapValue : α → β)
    (factor : NNRat) (weights : List (α × NNRat)) :
    mapWeights mapValue (scaleWeights factor weights) =
      scaleWeights factor (mapWeights mapValue weights) := by
  induction weights with
  | nil => rfl
  | cons row rest inner => simp only [scaleWeights, mapWeights, inner]

private theorem mapWeights_bindWeights {α : Type u} {β : Type v} {γ : Type w}
    (kernel : α → FiniteMeasure β) (mapValue : β → γ) (weights : List (α × NNRat)) :
    mapWeights mapValue (bindWeights kernel weights) =
      bindWeights (fun value => map mapValue (kernel value)) weights := by
  induction weights with
  | nil => rfl
  | cons row rest inner =>
      simp only [bindWeights, mapWeights_append, mapWeights_scaleWeights, inner]
      rfl

/-- Mapping a bind's result maps each row's kernel. -/
theorem map_bind_eq {α : Type u} {β : Type v} {γ : Type w} (measure : FiniteMeasure α)
    (kernel : α → FiniteMeasure β) (mapValue : β → γ) :
    map mapValue (bind measure kernel) = bind measure fun value => map mapValue (kernel value) :=
  congrArg FiniteMeasure.mk (mapWeights_bindWeights kernel mapValue measure.weights)

end FiniteMeasure

/-- `Rat`'s maximum is lawful for its order. Core states this for `min`, whose
`minOfLe` is left-leaning by construction, but not for `max`, whose `maxOfLe`
returns `b` when `a ≤ b` and so matches no instance Std derives. With it,
Std's `max` lemmas apply to the rational endpoints of intervals. -/
instance instLawfulOrderMaxRat : Std.LawfulOrderMax Rat where
  max_eq_or a b := by
    simp only [max]
    split
    · exact .inr rfl
  ...
  max_le_iff a b c := by
    simp only [max]
    split
    · exact ⟨fun below => ⟨Rat.le_trans ‹a ≤ b› below, below⟩, fun both => both.2⟩
    · exact ⟨fun below => ⟨below, Rat.le_trans (Rat.le_of_lt (Rat.not_le.mp ‹¬a ≤ b›)) below⟩,
        fun both => both.1⟩
