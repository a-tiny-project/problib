...
theorem toMeasure_scale (space : Space alpha) (factor : NNRat) (measure : FiniteMeasure alpha) :
  ...
  | mk weights =>
    ...
    | nil =>
      ...
      rw [Measure.smul_apply_measurable _ _ measurable, Measure.zero_apply, ENNReal.mul_zero]
    | cons head tail induction =>
  ...
  | mk weights =>
    ...
    | cons head tail induction =>
      ...
      rw [Measure.add_apply_measurable _ _ space.univ,
        ...
        ENNReal.mul_one, induction, NNRat.toENNReal_add]
...
/-- Normalizing a table and then pushing it along a map interprets as the
normalization of the pushed table. The map keeps the total, so the two
normalizing constants are one number. -/
theorem toMeasure_normalize_map (space : Space beta) (measure : FiniteMeasure alpha)
    (function : alpha → beta) (nonzero : measure.total ≠ 0) :
    ((FinitePMF.normalize measure nonzero).measure.map function).toMeasure space =
      Measure.normalize ((measure.map function).toMeasure space)
        ((measure.map function).toMeasure_isNormalizable space
          (by rw [total_map]; exact nonzero)) := by
  have mappedNonzero : (measure.map function).total ≠ 0 := by
    rw [total_map]
    exact nonzero
  rw [← toMeasure_normalize space _ mappedNonzero]
  apply toMeasure_eq_of_equivalent
  intro integrand
  change ((measure.scale (NNRat.inverse measure.total nonzero)).map function).integral integrand =
    ((measure.map function).scale
      (NNRat.inverse (measure.map function).total mappedNonzero)).integral integrand
  have inverse : NNRat.inverse (measure.map function).total mappedNonzero =
      NNRat.inverse measure.total nonzero := by
    have total := total_map function measure
    revert mappedNonzero
    rw [total]
    intro _
    rfl
  rw [integral_map, integral_scale, integral_scale, integral_map, inverse]
