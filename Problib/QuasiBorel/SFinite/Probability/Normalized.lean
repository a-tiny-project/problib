...
import Problib.QuasiBorel.SFinite.Standard.Family
...
theorem Standard.Generator.totalRandom_accepted (generator : Standard.Generator.{u, v} space)
  ...
  (Space.copair (Hom.constant (Space.terminal realSource) space (generator.random fallback))
    (Hom.identity space)).map_random generator.toPartial.accepted
...
noncomputable def toProbabilityHom (space : Space.{0, u} realSource) :
  ...
  map_random := by
    ...
    let fallback := Classical.choice (normalized Problib.Real.Construction.Dedekind.zero).nonempty
    refine ⟨{
...
@[simp] theorem inclusion_of_probability (space : Space.{0, u} realSource) :
...
theorem ofProbability_random_iff (laws : Carrier → Probability.Law space) :
  ...
  · intro accepted
    have normalized : (Normalized.object space).Random (fun seed => Normalized.ofProbability (laws seed)) :=
      accepted
    have recovered := (Normalized.toProbabilityHom space).map_random normalized
    ...
    exact recovered
  · intro accepted
    exact (ofProbabilityHom space).map_random accepted
