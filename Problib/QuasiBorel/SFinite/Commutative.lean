...
public import Problib.QuasiBorel.SFinite.Standard.Bind
...
private theorem bindContext_source (first : Standard.Presentation.{u, x} left)
...
theorem Standard.Presentation.bind_comm (first : Standard.Presentation.{u, x} left)
  ...
  rw [bindContext_source first second family @measurable,
    bindContext_source second first (Hom.comp family (Space.swap right left)) @measurable]
  let joint : Hom (Space.ofMeasurable borel (Problib.Measure.Space.product first.source second.source))
      (Space.product left right) :=
    Space.pair
      (Hom.comp first.random (Hom.ofMeasurable
        (Problib.Measure.Space.first_measurable first.source second.source)))
      (Hom.comp second.random (Hom.ofMeasurable
        (Problib.Measure.Space.second_measurable first.source second.source)))
  have jointMeasurable : MeasurableMap
      (Problib.Measure.Space.product first.source second.source)
      (Space.product left right).toMeasurable joint :=
    joint.toMeasurable_of_embedding (first.standard.product second.standard).embeddingReal
  have integrand : ENNRealMeasurable (Problib.Measure.Space.product first.source second.source)
