...
public import Problib.QuasiBorel.SFinite.Standard.Family
...
structure JointFamily (space : Space.{0, u} realSource) (laws : Carrier → Law space) where
  ...
  random : Hom (Space.ofMeasurable borel (Problib.Measure.Space.product borel source)) space
  kernel : Kernel borel source
  sfinite : Kernel.IsSFinite kernel
  law : ∀ seed, (kernel seed).map (fun point => random (seed, point))
    (MeasurableMap.comp
      (random.toMeasurable_of_embedding (StandardBorel.real.product standard).embeddingReal)
...
@[expose] noncomputable def toShared (family : JointFamily.{u, v} space laws) :
  ...
  source := Problib.Measure.Space.product borel family.source
...
@[expose] noncomputable def ofShared (family : Family.{u, v} space laws) :
  ...
  random := Hom.comp family.random
    (Hom.ofMeasurable (Problib.Measure.Space.second_measurable borel family.source))
