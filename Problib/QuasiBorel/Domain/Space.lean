...
public import Problib.Domain.Function
public import Problib.QuasiBorel.CartesianClosed
public import Problib.QuasiBorel.Pi
public import Problib.QuasiBorel.Subtype

namespace Problib.QuasiBorel.Domain
...
structure Predomain {Ω : Type u} (source : Source Ω) where
  ...
  random_sup : ∀ (c : Nat → Ω → space.Carrier)
...
@[expose] def unrestricted (source : Source Ω) {α : Type v} (D : CPO α) : Predomain source where
  ...
  order := D
  random_sup := fun _ _ _ => True.intro
...
@[expose] def product (D E : Predomain source) : Predomain source where
  ...
  random_sup := fun c hc hr =>
    ⟨D.random_sup (fun n seed => (c n seed).1) (fun seed i j h => (hc seed i j h).1)
      (fun n => (hr n).1),
     E.random_sup (fun n seed => (c n seed).2) (fun seed i j h => (hc seed i j h).2)
      (fun n => (hr n).2)⟩
...
@[expose] def pi {ι : Type w} (D : ι → Predomain source) : Predomain source where
  ...
  random_sup := fun c hc hr i => (D i).random_sup (fun n seed => c n seed i)
