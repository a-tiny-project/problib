...
public import Problib.QuasiBorel.Domain.Space
...
@[expose] def homSup (D E : Predomain source) (c : Nat → Hom D E)
  ...
  ⟨{ toFun := fun x => E.order.sup (fun n => c n x) (hc x)
     map_random := fun {random} hr => E.random_sup (fun n seed => c n (random seed))
       (fun seed => hc (random seed)) (fun n => (c n).val.map_random hr) },
...
@[expose] def homOrder (D E : Predomain source) : CPO (Hom D E) where
  ...
  below_sup := fun c _ n x => E.order.below_sup (fun n => c n x) _ n
  sup_least := fun _ _ b h x => E.order.sup_least _ _ (b x) (fun n => h n x)
...
@[expose] def exponential (D E : Predomain source) : Predomain source where
  ...
  random_sup := by
    ...
    exact E.random_sup (fun n seed => (c n (pair seed).1).val.toFun (pair seed).2)
