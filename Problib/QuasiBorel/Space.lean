...
public import Problib.QuasiBorel.Source
...
structure Hom {Ω : Type u} {source : Source Ω}
  ...
  map_random : ∀ {random}, domain.Random random →
...
@[expose] def constant {Ω : Type u} {source : Source Ω}
  ...
  toFun := fun _ => point
  map_random := fun _ => codomain.constant point
...
@[expose] def comp {Ω : Type u} {source : Source Ω} {first second third : Space source}
  ...
  map_random := fun random => after.map_random (before.map_random random)
...
@[expose] def pair {Ω : Type u} {source : Source Ω} {domain left right : Space source}
  ...
  map_random := fun random => ⟨leftMap.map_random random, rightMap.map_random random⟩
