...
@[reducible, expose] def exponential {Ω : Type u} {source : Source Ω}
  ...
  constant := by
    ...
    exact morphism.map_random randomPairValid.2
  reparam := by
  ...
  piecewise := by
    ...
    exact codomain.piecewise
      (source.partition_reparam partitionMeasurable randomPairValid.1)
...
@[expose] def curry {Ω : Type u} {source : Source Ω} {parameter domain codomain : Space source}
  ...
  toFun := fun parameterValue => {
    ...
    map_random := fun domainRandom =>
      morphism.map_random ⟨parameter.constant parameterValue, domainRandom⟩
  }
  map_random := by
    intro parameterRandom parameterRandomValid randomPair randomPairValid
    apply morphism.map_random
