...
theorem from_sum_iff {left : Space α} {right : Space β} {target : Space γ}
    {function : Sum α β → γ} :
...
theorem sum_elim {left : Space α} {right : Space β} {target : Space γ}
  ...
  from_sum_iff.mpr ⟨leftMeasurable, rightMeasurable⟩

/-- Proves that case elimination is measurable if and only if both branch maps are measurable. -/
theorem sum_elim_iff {left : Space α} {right : Space β} {target : Space γ}
    {leftMap : α → γ} {rightMap : β → γ} :
    MeasurableMap (Space.sum left right) target (Sum.elim leftMap rightMap) ↔
...
theorem sum_map {first : Space α} {second : Space β} {third : Space γ} {fourth : Space δ}
  ...
  sum_elim (comp (inl third fourth) leftMeasurable)
