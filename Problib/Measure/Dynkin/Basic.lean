...
private theorem pairFamily_pairwise {left right : Set α}
...
private theorem iUnion_pairFamily (left right : Set α) :
...
theorem union {system : DynkinSystem α} {left right : Set α}
  ...
  rw [← iUnion_pairFamily left right]
  apply system.iUnion (pairFamily_pairwise disjoint)
  intro index
  cases index with
...
private theorem union_of_intersection_closed (system : DynkinSystem α)
...
private theorem prefixUnion_contains (system : DynkinSystem α)
    (intersection : ∀ {left right}, system.Contains left →
      system.Contains right → system.Contains (Set.inter left right))
  ...
  | succ count induction =>
      exact union_of_intersection_closed system intersection induction
        (contains count)
  ...
  iUnion := by
    ...
    have piecesContain :
      ...
      exact intersection (contains index)
        (system.complement
          (prefixUnion_contains system intersection contains index))
