...
namespace Problib.Measure.Coding.Unit
...
public theorem selected_bits_le (point : UnitInterval) (index : Nat) :
  ...
  split
  · rename_i included
    exact ENNReal.ofReal_monotone included
...
public theorem value_bits (point : UnitInterval) : value (bits point) = point := by
  ...
  · have included := unitClamp_mono (ENNReal.iSup_le (selected_bits_le point))
    rw [unitClamp_ofReal] at included
    exact included
  · apply Classical.byContradiction
    intro missing
    have less := not_le_iff_lt.mp missing
    ...
    have included := ENNReal.le_iSup (selected (bits point)) index
...
public theorem bits_measurable (index : Nat) :
  ...
  rw [equal]
  exact unitInclusion_measurable (measurable_ici (unitRationalBasis index).val)
...
public theorem value_measurable {α : Type u} {source : Space α} {digits : α → Nat → Bool}
  ...
  exact (ENNRealMeasurable.iSup selections).measurableMap (unitClamp_measurable measurable)

end Problib.Measure.Coding.Unit
