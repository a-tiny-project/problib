...
private theorem difference_empty (set : Set α) :
...
private theorem difference_complement (set region : Set α) :
...
private theorem inter_union_split (set left right : Set α) :
...
private theorem difference_union (set left right : Set α) :
...
public theorem isCaratheodory_empty (outer : OuterMeasure α) :
  ...
  intro set
  rw [Set.inter_empty_right, difference_empty, outer.empty_apply, ENNReal.zero_add]
  exact ENNReal.le_refl _
...
private theorem inter_union_apply (outer : OuterMeasure α)
...
public theorem partialSum_inter_eq (outer : OuterMeasure α)
  ...
  | 0 => by rw [ENNReal.partialSum, Set.prefixUnion, Set.inter_empty_right,
      outer.empty_apply]
  | count + 1 => by
      rw [ENNReal.partialSum,
        outer.partialSum_inter_eq measurable disjoint set count,
        Set.prefixUnion]
      exact (inter_union_apply outer
