...
namespace Problib.Measure.Necessity.Disintegration.Probability

open Problib.Measure
open Problib.Real

open Problib.Measure.Necessity
...
theorem conditional_univ (input : Unit) : conditional input Set.univ = Density.two := by
  ...
  rw [Measure.smul_apply_measurable _ _ True.intro, Measure.dirac_apply_univ, ENNReal.mul_one]
...
theorem joint_univ : joint Set.univ = ENNReal.top := by
  ...
  rw [equal, lintegral_const, Density.reference_univ,
    ENNReal.mul_top_of_ne_zero Density.two_ne_zero]
...
theorem not_probability_ae : ¬(Measure.secondMarginal joint).AE
  ...
  exact ENNReal.top_ne_finite NNReal.zero probability
...
theorem s_finite_disintegration_not_probability_ae :
...
end Problib.Measure.Necessity.Disintegration.Probability
