...
public import Problib.Measure.Integral.Density.Finite
...
private noncomputable def weightedTopZeroInfinity {reference : Measure space}
  ...
  refine { set := infinity, zero_infinity := ⟨infinityMeasurable, ?_⟩, greatest := ?_ }
...
private noncomputable def decomposition {target : Measure space} (finite : SFinite target) :
  ...
  let derivative := RadonNikodymDerivative.ofSFinite finite
    (SigmaFinite.ofFinite reference.finite) reference.target_continuous
  have result := weightedDecomposition reference.finite derivative.density_measurable
...
public theorem TopZeroInfinitySet.null_iff_sigmaFinite {target : Measure space}
  ...
  · intro null
    ...
    exact ⟨cover⟩
  · rintro ⟨cover⟩
    exact top.zero_infinity.null_of_sigmaFinite cover
