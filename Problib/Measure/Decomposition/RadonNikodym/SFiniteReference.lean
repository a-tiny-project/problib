...
public theorem zeroInfinityAbsolutelyContinuous {target reference : Measure space}
  ...
  derivative.reconstruct.zeroInfinityAbsolutelyContinuous derivative.density_measurable
...
public noncomputable def ofSFiniteReference {target reference : Measure space}
  ...
  let region := top.set
  have regionMeasurable : space.Measurable region := top.zero_infinity.measurable
  have targetRegion : IsZeroInfinitySet target region := continuous.preserves_zero_infinity top.zero_infinity
  ...
  let supportDerivative := RadonNikodymDerivative.ofSFinite targetFinite
    ...
    (AbsolutelyContinuous.trans continuous.absolutelyContinuous finiteReference.target_continuous)
  let support := fun value => supportDerivative.density value ≠ ENNReal.zero
  have supportMeasurable : space.Measurable support :=
    space.complement (supportDerivative.density_measurable.eq_set
  ...
  have insideReconstruct : target.restrict region = (reference.restrict region).withDensity inside := by
    ...
    have zeroIff : target (Set.inter set region) = ENNReal.zero ↔
      ...
      exact (withDensity_apply_eq_zero_iff supportDerivative.density_measurable
    ...
    · rcases top.zero_infinity.subsets_zero_or_top
  ...
  refine {
    ...
    density_measurable := ENNRealMeasurable.piecewise regionMeasurable insideMeasurable
      outside.density_measurable
    reconstruct := ?_
  }
  change target = reference.withDensity (ennrealPiecewise region inside outside.density)
  rw [reference.withDensity_piecewise regionMeasurable insideMeasurable outside.density_measurable,
