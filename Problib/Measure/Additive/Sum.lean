...
@[expose] public noncomputable def sum
  ...
  empty := by
    calc
      ...
      _ = ENNReal.zero := ENNReal.tsum_zero
    ...
    calc
      ...
      _ = ENNReal.tsum (fun setIndex =>
          ENNReal.tsum (fun measureIndex =>
            (measures measureIndex).content (sets setIndex)
              (measurable setIndex))) :=
        ENNReal.tsum_comm fun measureIndex setIndex =>
          (measures measureIndex).content (sets setIndex)
            (measurable setIndex)
...
public theorem le_sum (measures : Nat → Measure source) (index : Nat) (set : Set α) :
  ...
  exact ENNReal.term_le_tsum (fun stage => measures stage region) index
...
public theorem sum_reindex (equivalence : Countable.Bijection Nat Nat)
  ...
  exact ENNReal.tsum_reindex equivalence (fun index => measures index set)
...
public theorem sum_comm (measures : Nat → Nat → Measure source) :
  ...
  exact ENNReal.tsum_comm fun first second => measures first second set
...
public theorem sum_double (measures : Nat → Nat → Measure source) :
  ...
  rw [rows]
  exact ENNReal.tsum_flatten fun row column => measures row column set
...
public theorem sum_const (measure : Measure source) :
  ...
  · rw [zero, ENNReal.mul_zero, ENNReal.tsum_zero]
  · rw [ENNReal.tsum_const_of_ne_zero zero, ENNReal.top_mul_of_ne_zero zero]
