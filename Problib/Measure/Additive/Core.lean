...
public import Problib.Measure.Caratheodory
...
private theorem tsum_pairValues (left right : ENNReal) :
  ...
  rw [equal, ENNReal.tsum_add, ENNReal.tsum_single, ENNReal.tsum_single]
...
public theorem content_mono (μ : Measure space) {left right : Set α}
  ...
  have shifted := ENNReal.add_le_add_left
    (ENNReal.zero_le (μ.content (Set.difference right left)
      differenceMeasurable)) (μ.content left leftMeasurable)
  simpa only [ENNReal.add_zero] using shifted
...
private theorem extendedContent_iUnion_le (μ : Measure space)
  ...
  · rw [μ.extendedContent_apply_measurable (space.iUnion measurable)]
    ...
    exact ENNReal.le_trans included
  ...
  · have missing : ∃ index, ¬space.Measurable (sets index) := by
    ...
    have topIncluded := ENNReal.term_le_tsum
    ...
    exact ENNReal.le_trans (ENNReal.le_top _) topIncluded
...
public theorem apply_measurable (μ : Measure space) {set : Set α}
    ...
    μ set = μ.content set measurable := by
  calc
...
@[simp] public theorem empty_apply (μ : Measure space) :
  ...
  rw [μ.apply_measurable space.empty, μ.empty]
