...
public theorem pair_left_measurable (input : alpha) :
  ...
  Space.pair_measurable (Kernel.vertical_map_measurable input)
...
public theorem prod {left : Measure source} {right : Measure target}
  ...
  rcases ENNReal.exists_finite_of_finite leftFinite.univ_finite with
    ⟨leftValue, leftEqual⟩
  rcases ENNReal.exists_finite_of_finite rightFinite.univ_finite with
