...
public import Problib.Measure.Kernel.Hahn.Countable
public import Problib.Measure.Kernel.Scale
public import Problib.Measure.Decomposition.RadonNikodym.Finite
import Problib.Measure.Decomposition.Hahn.Density
import Problib.Measure.Decomposition.Hahn.Levels
...
public noncomputable def ofFiniteFibers (generator : Space.CountableGenerator target)
  ...
  let decompositions : ∀ index, HahnDecomposition kernel (Kernel.smul (ENNReal.rationalBasis index) reference) :=
    fun index => HahnDecomposition.ofCountableGenerator generator kernelFinite
      (fun input => (referenceFinite input).smul (ENNReal.rationalBasis index) (ENNReal.rationalBasis_finite index))
  ...
  refine { density := density, density_measurable := measurable, reconstruct := ?_ }
  intro input
  ...
  have bounds (index : Nat) : (reference input).AE (fun value =>
    ...
    let constant : Measure.RadonNikodymDerivative
      ...
      density_measurable := ENNRealMeasurable.constant target _
