...
public noncomputable def ofCountableSource (code : α → Nat)
  ...
  exact {
    ...
    density_measurable := ENNRealMeasurable.of_measurableMap
      (MeasurableMap.uncurry_of_countable_first code injective
        (fun input => (derivatives input).density_measurable.measurableMap))
...
reference fibers. -/
public theorem nonempty_iff_of_countable_source (code : α → Nat)
    (injective : Function.Injective code)
    (kernelFinite : ∀ input, Measure.SFinite (kernel input))
    (referenceFinite : ∀ input, Measure.SFinite (reference input)) :
