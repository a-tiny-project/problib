...
public theorem measure {kernel : Kernel source target}
  ...
  exact ⟨ENNReal.finite_of_le (bounded input) boundFinite⟩
...
public theorem const (source : Space alpha) {measure : Measure target}
  ...
  ⟨⟨measure Set.univ, finite.univ_finite,
    fun _ => ENNReal.le_refl _⟩⟩
...
public theorem add {left right : Kernel source target}
  ...
  exact ENNReal.add_le_add (leftBounded input) (rightBounded input)
