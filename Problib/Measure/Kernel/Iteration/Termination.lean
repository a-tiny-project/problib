...
public import Problib.Real.Extended.Infimum
...
public theorem prefix_mass_recurrence (step : Kernel source source)
  ...
  rw [prefixApproximant_step_unfold, Kernel.add_apply,
    ...
    comp_apply_measurable _ _ _ source.univ, ENNReal.add_assoc,
...
public theorem prefix_mass_le_one (step : Kernel source source)
  ...
  | zero =>
    rw [prefixApproximant_zero, Kernel.zero_apply, Measure.zero_apply,
      iterate_zero, deterministic_apply, Measure.dirac_apply_univ, ENNReal.zero_add]
    exact ENNReal.le_refl _
  | succ count induction =>
    rw [prefix_mass_recurrence]
    have integralBound := lintegral_mono (step input) induction
    rw [lintegral_const, ENNReal.one_mul] at integralBound
    exact ENNReal.le_trans (ENNReal.add_le_add_left integralBound _)
      (by rw [ENNReal.add_comm]; exact substochastic input)
...
public theorem prefix_mass_eq_one (step : Kernel source source)
  ...
  | succ count induction =>
    rw [prefix_mass_recurrence, lintegral_congr (step input) induction,
      lintegral_const, ENNReal.one_mul, ENNReal.add_comm]
...
public theorem loop_mass_le_one (step : Kernel source source)
  ...
  intro count
  have included := ENNReal.add_le_add_left (ENNReal.zero_le
    (iterate step count input Set.univ)) (prefixApproximant step exit count input Set.univ)
  rw [ENNReal.add_zero] at included
  exact ENNReal.le_trans included (prefix_mass_le_one step exit substochastic count input)
...
public theorem substochastic_isFinite (step : Kernel source source)
  ...
  · refine ⟨⟨ENNReal.one, True.intro, ?_⟩⟩
    intro input
    have included := ENNReal.add_le_add_left (ENNReal.zero_le (exit input Set.univ))
      (step input Set.univ)
    rw [ENNReal.add_zero] at included
    exact ENNReal.le_trans included (substochastic input)
  · refine ⟨⟨ENNReal.one, True.intro, ?_⟩⟩
    intro input
    have included := ENNReal.add_le_add_right (ENNReal.zero_le (step input Set.univ))
      (exit input Set.univ)
...
public theorem iterate_mass_antitone (step : Kernel source source)
  ...
  | succ second induction =>
    ...
    · rw [equal]
      exact ENNReal.le_refl _
    · exact ENNReal.le_trans (successor second) (induction (by omega))
...
public theorem loop_mass_eq_one_sub_iInf (step : Kernel source source)
  ...
  rw [loop_eq_iSup_prefixApproximant step exit input target.univ, ENNReal.sub_iInf]
  apply congrArg ENNReal.iSup
  funext count
  have balance := prefix_mass_eq_one step exit conservative count input
  have residualBound := ENNReal.add_le_add_right
    (ENNReal.zero_le (prefixApproximant step exit count input Set.univ))
    (iterate step count input Set.univ)
  rw [ENNReal.zero_add, balance] at residualBound
  have finite := ENNReal.finite_of_le residualBound (show ENNReal.Finite ENNReal.one from True.intro)
  rw [← balance, ENNReal.add_sub_cancel_right finite]
...
public theorem loop_mass_add_iInf_eq_one (step : Kernel source source)
  ...
  apply ENNReal.sub_add_cancel
  have initial := ENNReal.iInf_le (fun count => iterate step count input Set.univ) 0
...
public theorem loop_mass_one_iff (step : Kernel source source)
  ...
  · intro outputOne
    ...
    apply ENNReal.add_left_cancel_of_finite (show ENNReal.Finite ENNReal.one from True.intro)
    rw [ENNReal.add_zero]
    exact balance
  · intro vanishing
    rw [vanishing, ENNReal.add_zero] at balance
    exact balance
