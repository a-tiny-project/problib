...
public theorem IsFinite.piecewise {left right : Kernel source target}
  ...
  refine ⟨⟨ENNReal.add leftBound rightBound,
    ENNReal.add_finite leftBoundFinite rightBoundFinite, ?_⟩⟩
  intro input
  by_cases member : region input
  · rw [piecewise_apply_of_mem _ _ _ _ _ member]
    apply ENNReal.le_trans (leftBounded input)
    simpa only [ENNReal.add_zero] using
      ENNReal.add_le_add_left (ENNReal.zero_le rightBound) leftBound
  · rw [piecewise_apply_of_not_mem _ _ _ _ _ member]
    apply ENNReal.le_trans (rightBounded input)
    simpa only [ENNReal.zero_add] using
      ENNReal.add_le_add_right (ENNReal.zero_le leftBound) rightBound
