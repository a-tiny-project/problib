...
public theorem lintegral_withDensity
  ...
  exact Problib.Measure.lintegral_withDensity (kernel input)
    (jointly_measurable_slice densityMeasurable input) integrandMeasurable
  ...
  calc
    ...
    _ = ENNReal.tsum (fun index =>
      ...
      lintegral_tsum ((kernel input).restrict set)
        ...
        (fun index => jointly_measurable_slice
...
public theorem withDensity_of_bounded {kernel : Kernel source target}
  ...
  refine ⟨⟨ENNReal.mul bound kernelBound,
    ENNReal.mul_finite boundFinite kernelBoundFinite, ?_⟩⟩
  intro input
  ...
  exact ENNReal.le_trans
    (lintegral_mono (kernel input) (bounded input)) (by
      rw [lintegral_const]
      exact ENNReal.mul_le_mul_left (kernelBounded input) bound)
