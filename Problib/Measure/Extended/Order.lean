...
public import Problib.Measure.Extended.Basic
...
private theorem not_lt_iff_le (left right : ENNReal) :
    (¬ENNReal.lt left right) ↔ ENNReal.le right left := by
  constructor
  · intro notLess
    rcases ENNReal.le_total right left with reverse | forward
    · exact reverse
...
private theorem not_le_iff_lt (left right : ENNReal) :
    (¬ENNReal.le left right) ↔ ENNReal.lt right left := by
  constructor
  · intro notIncluded
    refine ⟨?_, notIncluded⟩
    exact Or.resolve_left (ENNReal.le_total left right) notIncluded
  · intro less included
...
public theorem iic (measurable : ENNRealMeasurable source function)
  ...
  have equal : Set.preimage function (ennrealIic threshold) =
    ...
    exact Iff.symm (not_lt_iff_le threshold (function value))
  rw [equal]
  ...
  have equal : Set.preimage function (ennrealIio threshold) =
    ...
    · rintro ⟨index, member⟩
      ...
      · simp only [sets, if_pos below] at member
        refine ⟨ENNReal.le_trans member below.left, ?_⟩
        intro thresholdValue
        exact below.right (ENNReal.le_trans thresholdValue member)
...
public theorem ici (measurable : ENNRealMeasurable source function)
  ...
  have equal : Set.preimage function (ennrealIci threshold) =
    ...
    exact Iff.symm (not_lt_iff_le (function value) threshold)
  rw [equal]
  exact complementMeasurable
...
public theorem singleton (measurable : ENNRealMeasurable source function)
  ...
  have equal : Set.preimage function (ennrealSingleton target) =
    ...
    · intro equal
      rw [equal]
      exact ⟨ENNReal.le_refl target, ENNReal.le_refl target⟩
    · intro both
      exact ENNReal.le_antisymm both.1 both.2
  rw [equal]
  exact intersection
...
public theorem lt_set (leftMeasurable : ENNRealMeasurable source left)
  ...
  have equal : (fun value => ENNReal.lt (left value) (right value)) =
    ...
    · rintro ⟨index, leftBasis, basisRight⟩
      refine ⟨ENNReal.le_trans leftBasis basisRight.left, ?_⟩
      intro rightLeft
      exact basisRight.right (ENNReal.le_trans rightLeft leftBasis)
  rw [equal]
  exact source.iUnion setsMeasurable
...
public theorem le_set (leftMeasurable : ENNRealMeasurable source left)
  ...
  have equal : (fun value => ENNReal.le (left value) (right value)) =
    ...
    apply Set.ext
    intro value
    exact Iff.symm (not_lt_iff_le (right value) (left value))
  rw [equal]
  exact complementMeasurable
...
public theorem eq_set (leftMeasurable : ENNRealMeasurable source left)
  ...
  have equal : (fun value => left value = right value) =
    ...
    · intro same
      rw [same]
      exact ⟨ENNReal.le_refl _, ENNReal.le_refl _⟩
    · intro both
      exact ENNReal.le_antisymm both.1 both.2
  rw [equal]
  exact intersection
...
public theorem iSup {functions : Nat → α → ENNReal}
  ...
  have equal : Set.preimage
    ...
    constructor
    · intro less
      exact ENNReal.exists_index_greater_of_lt_iSup less
    · rintro ⟨index, less⟩
      refine ⟨ENNReal.le_trans less.left
          (ENNReal.le_iSup (fun position => functions position value) index), ?_⟩
      intro supremumThreshold
      exact less.right (ENNReal.le_trans
        (ENNReal.le_iSup (fun position => functions position value) index)
        supremumThreshold)
  rw [equal]
...
public theorem iInf {functions : Nat → α → ENNReal}
  ...
  have same : Set.preimage (fun input => ENNReal.iInf (fun index => functions index input))
    ...
    constructor
    · intro less
      rcases ENNReal.exists_rationalBasis_between less with ⟨index, above, below⟩
      exact ⟨index, above, fun stage => ENNReal.le_trans below.1 (ENNReal.iInf_le _ stage)⟩
    · rintro ⟨index, above, below⟩
      have bound := ENNReal.le_iInf below
      exact ⟨ENNReal.le_trans above.1 bound, fun reverse => above.2 (ENNReal.le_trans bound reverse)⟩
  rw [same]
...
public theorem monotone_limit {functions : Nat → α → ENNReal}
    {limit : α → ENNReal}
    (measurable : ∀ index,
      ENNRealMeasurable source (functions index))
...
public theorem max {left right : α → ENNReal}
  ...
  have equal : (fun value => ENNReal.max (left value) (right value)) =
    ...
    · rw [ENNReal.max_eq_right included]
      simp only [ennrealPiecewise, if_pos included]
    · rw [ENNReal.max_eq_left ((ENNReal.le_total _ _).resolve_left included)]
