...
private theorem mul_approximation_left
...
public theorem mul
  ...
  have approximations : ∀ index, ENNRealMeasurable source (fun value =>
      ENNReal.mul (ENNReal.approximation (left value) index)
        (right value)) :=
    mul_approximation_left leftMeasurable rightMeasurable
  ...
  have equal : (fun value : α => ENNReal.mul (left value) (right value)) =
    ...
    calc
      ENNReal.mul (left value) (right value) =
          ENNReal.mul (right value) (left value) := ENNReal.mul_comm _ _
...
public theorem sub
  ...
  have same : Set.preimage (fun input => ENNReal.sub (left input) (right input))
    ...
    Set.ext fun _ => ENNReal.lt_sub_iff_add_lt
  rw [same]
  exact comparison
...
public theorem min
  ...
  have equal : (fun value : α => ENNReal.min (left value) (right value)) =
    ...
    · rw [ENNReal.min_eq_left leftRight]
      simp [ennrealPiecewise, included, leftRight]
    · have rightLeft := Or.resolve_left
        (ENNReal.le_total (left value) (right value)) leftRight
      rw [ENNReal.min_eq_right rightLeft]
      simp [ennrealPiecewise, included, leftRight]
  rw [equal]
  exact selected
