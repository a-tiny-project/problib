...
public import Problib.Measure.Approximation.Basic
...
public theorem iUnion (finite : IsFinite measure) (algebra : Space.CountableAlgebra space)
  ...
  have infimumLess : ENNReal.lt (ENNReal.iInf (fun index => measure (tails index)))
    ...
    exact halfPositive
  rcases ENNReal.exists_index_less_of_iInf_lt infimumLess with ⟨count, tailLess⟩
  ...
  have tailBound : ENNReal.le
    ...
    exact tailLess.1
  have bound := ENNReal.le_trans (measure.setDistance_triangle (Set.iUnion sets)
    (Set.prefixUnion sets count) (algebra.sets index))
    (ENNReal.add_le_add tailBound prefixBound)
