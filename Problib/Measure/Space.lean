...
theorem into_generated {source : Space α} {generators : Set (Set β)}
    {function : α → β}
...
theorem to_discrete (source : Space α) (function : α → β)
...
theorem from_discrete (target : Space β) (function : α → β) :
...
intersections. -/
theorem countable_piecewise {source : Space α} {target : Space β}
