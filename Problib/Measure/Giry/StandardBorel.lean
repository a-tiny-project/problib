...
import Problib.Measure.Kernel.Distribution.Bounds
import Problib.Measure.Kernel.Randomization.Unit
import Problib.Measure.StandardBorel.Pi

set_option autoImplicit false

namespace Problib.Measure.Giry

open Problib.Measure.Real (unitBorel UnitInterval unitZero unitRationalBasis)
...
noncomputable def unitInterval_standardBorel :
    Problib.Measure.StandardBorel (space unitBorel) := by
  refine Problib.Measure.StandardBorel.ofRealLeftInverse
...
noncomputable def standardBorel {alpha : Type u} {source : Space alpha}
    (presentation : Problib.Measure.StandardBorel source) :
    Problib.Measure.StandardBorel (space source) := by
  classical
  ...
  · apply Problib.Measure.StandardBorel.ofEmpty
    rintro ⟨law⟩
