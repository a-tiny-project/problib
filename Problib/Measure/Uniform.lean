...
public import Problib.Measure.Space.Indicator
...
public theorem uniform01_unitInitial (upper : UnitInterval) :
  ...
  have region : Set.inter (Iic upper.val) unitSet = Icc Dedekind.zero upper.val := by
    ...
    exact ⟨fun member => ⟨member.2.1, member.1⟩,
      fun member => ⟨member.2, member.1, Dedekind.le_trans member.2 upper.property.2⟩⟩
  calc
    uniform01 (unitInitial upper) =
        (uniform01.map unitInclusion unitInclusion_measurable) (Iic upper.val) :=
      (Measure.map_apply uniform01 unitInclusion unitInclusion_measurable
    ...
    _ = ENNReal.ofReal upper.val := by
      rw [volume_icc, Dedekind.sub_eq_add_neg, Dedekind.neg_zero, Dedekind.add_zero]
...
theorem uniform01_singleton (point : UnitInterval) :
  ...
  have transported := Measure.map_apply uniform01 unitInclusion unitInclusion_measurable
...
theorem uniform01_strict_initial (upper : UnitInterval) :
  ...
  have same : uniform01 (fun value => Dedekind.lt value.val upper.val) =
    ...
    · apply Measure.NullSet.mono
      ...
      intro value member
      apply Subtype.ext
      exact Dedekind.le_antisymm member.1 (not_lt_iff_le.mp member.2)
...
theorem unitThreshold_measurable (upper : UnitInterval) :
  ...
  boolIndicator_measurable (unitInclusion_measurable (measurable_iio upper.val))
...
/-- The uniform measure of the samples below a real threshold at most one is the
threshold clamped at zero. A threshold in the unit interval is an initial
interval, and a negative one holds no sample. -/
theorem uniform01_real_initial (threshold : Carrier)
    (atMostOne : Dedekind.le threshold Dedekind.one) :
    uniform01 (fun value => Dedekind.lt value.val threshold) = ENNReal.ofReal threshold := by
  classical
  by_cases nonnegative : Dedekind.le Dedekind.zero threshold
  · exact uniform01_strict_initial ⟨threshold, nonnegative, atMostOne⟩
  · have empty : (fun value : UnitInterval => Dedekind.lt value.val threshold) = Set.empty := by
      apply Set.ext
      intro value
      exact ⟨fun below => nonnegative (Dedekind.le_trans value.property.1 below.1),
        fun member => member.elim⟩
    rw [empty, Measure.empty_apply, ENNReal.ofReal_of_not_nonnegative nonnegative]
