...
public import Problib.Measure.Equivalence
public import Problib.Measure.Set.Image
...
public structure MeasurableEmbedding {α : Type u} {β : Type v}
  ...
  image_measurable : ∀ region, source.Measurable region →
...
@[expose] public def trans (before : MeasurableEmbedding source middle)
  ...
  image_measurable := by
    intro region measurable
    rw [Set.image_comp]
    exact after.image_measurable _ (before.image_measurable region measurable)
...
public theorem range_measurable (embedding : MeasurableEmbedding source middle) :
  ...
  rw [← Set.image_univ]
  exact embedding.image_measurable Set.univ source.univ
    ...
    inverse_measurable := by
      ...
      have equal : Set.preimage inverse region =
        ...
        · rintro ⟨value, member, equal⟩
          ...
          exact same ▸ member
      rw [equal]
      exact Space.comap_map _ middle (embedding.image_measurable region measurable)
  }
