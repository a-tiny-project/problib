...
import Problib.Measure.Integral.Simple.Integral.Add
...
public theorem lintegral_zero (measure : Measure space) :
  ...
  rw [lintegral_const, ENNReal.zero_mul]
...
/-- The mass of a measurable set is the integral of its indicator. -/
public theorem apply_eq_lintegral_indicator (measure : Measure space)
    {region : Set alpha} (regionMeasurable : space.Measurable region) :
    measure region = lintegral measure (ennrealIndicator region fun _ => ENNReal.one) := by
  rw [lintegral_indicator measure region regionMeasurable, lintegral_const,
    Measure.restrict_apply_univ, ENNReal.one_mul]
...
public theorem lintegral_smul (measure : Measure space) (factor : ENNReal)
  ...
  have pointwise : ∀ input,
    ...
    calc
      ...
      _ = ENNReal.iSup (fun index => ENNReal.mul factor
        ...
        ENNReal.mul_iSup factor (fun index => approximations index input)
  ...
  calc
    ...
    _ = ENNReal.mul factor (ENNReal.iSup (fun index =>
      ...
      (ENNReal.mul_iSup factor (fun index =>
...
public theorem lintegral_tsum (measure : Measure space)
  ...
  have sumsMonotone : ∀ count input,
    ...
    fun count input => ENNReal.partialSum_step
