...
public theorem withDensity_piecewise (measure : Measure space)
  ...
  have equal : ennrealPiecewise region inside outside =
    ...
    · simp only [ennrealIndicator, ennrealPiecewise, Set.complement, member,
        if_true, not_true_eq_false, if_false, ENNReal.add_zero]
    · simp only [ennrealIndicator, ennrealPiecewise, Set.complement, member,
        if_true, not_false_eq_true, if_false, ENNReal.zero_add]
...
/-- Normalizing a weighted measure weights the base measure by the density over
its total mass. With a finite nonzero mass `∫ w`, the normalized law has density
`w / ∫ w` against the unweighted measure. -/
public theorem withDensity_normalize (measure : Measure space) {weight : alpha → ENNReal}
    (weightMeasurable : ENNRealMeasurable space weight)
    (defined : IsNormalizable (measure.withDensity weight)) {mass : NNReal}
    (total : lintegral measure weight = ENNReal.finite mass) :
    normalize (measure.withDensity weight) defined =
      measure.withDensity (fun value =>
        ENNReal.mul (weight value) (ENNReal.finite (NNReal.div NNReal.one mass))) := by
  have weightedTotal : measure.withDensity weight Set.univ = ENNReal.finite mass := by
    rw [measure.withDensity_apply _ space.univ, measure.restrict_univ]
    exact total
  rw [normalize_eq_of_total_eq _ defined weightedTotal]
  apply Measure.ext
  intro set setMeasurable
  rw [Measure.smul_apply_measurable _ _ setMeasurable, measure.withDensity_apply _ setMeasurable,
    measure.withDensity_apply _ setMeasurable, ← lintegral_smul _ _ weightMeasurable]
