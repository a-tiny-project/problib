...
public theorem aeEq_of_withDensity_eq {left right : alpha → ENNReal}
  ...
  exact (forward.and reverse).mono
    (fun _ both => ENNReal.le_antisymm both.1 both.2)
...
public theorem withDensity_eq_iff_aeEq {left right : alpha → ENNReal}
    (finite : SigmaFinite measure)
    (leftMeasurable : ENNRealMeasurable space left)
    (rightMeasurable : ENNRealMeasurable space right) :
    measure.withDensity left = measure.withDensity right ↔
      measure.AEEq left right :=
  ⟨aeEq_of_withDensity_eq finite leftMeasurable rightMeasurable,
